...
RGraph.Bar = function(conf) {
    var id = conf.id, canvas = document.getElementById(id), data = conf.data;
    ...
    //
    for(var i1 = 0; i1 < data.length; ++i1)if (typeof data[i1] === "object" && !RGraph.isNull(data[i1])) this.stackedOrGrouped = true;
    //
    // Create the dollar objects so that functions can be added to them
    //
    var linear_data = RGraph.arrayLinearize(data);
    for(var i1 = 0; i1 < linear_data.length; ++i1)this["$" + i1] = {};
    ...
    this.set = function(name) {
        ...
        if (arguments.length === 1 && typeof arguments[0] === "object") {
            for(i1 in arguments[0])if (typeof i1 === "string") this.set(i1, arguments[0][i1]);
            return this;
        }
    ...
    this.draw = function() {
        ...
        //
        if (properties.xaxisPosition === "top") for(var i1 = 0; i1 < this.data.length; ++i1){
            if (typeof this.data[i1] === "object" && properties.grouping === "grouped") for(var j = 0; j < this.data[i1].length; ++j)this.data[i1][j] = Math.abs(this.data[i1][j]) * -1;
            else if (typeof this.data[i1] === "number") this.data[i1] = Math.abs(this.data[i1]) * -1;
        }
        //
        ...
        if (properties.variant === "3d") {
            ...
            else this.context.setTransform(1, properties.variantThreedAngle, 0, 1, 0.5, 0.5);
        }
        //
        ...
        // with pyramid charts
        //
        if ((properties.variant == "pyramid" || properties.variant == "dot") && typeof properties.tooltips == "object" && properties.tooltips && properties.tooltips.length > 0) alert("[BAR] (" + this.id + ") Sorry, tooltips are not supported with dot or pyramid charts");
        //
        ...
        //
        if (properties.beveled || properties.beveled) this.drawBevel();
    ...
    this.drawAxes = function() {
        ...
        if (properties.xaxisLabels && properties.xaxisLabels.length) {
            if (typeof properties.xaxisLabels === "string") properties.xaxisLabels = RGraph.arrayPad({
                array: [],
            ...
            // Label substitution
            //
            for(var i1 = 0; i1 < properties.xaxisLabels.length; ++i1)properties.xaxisLabels[i1] = RGraph.labelSubstitution({
                object: this,
                text: properties.xaxisLabels[i1],
                index: i1,
                value: this.data[i1],
    ...
    this.drawbars = function() {
        ...
        var prevX = 0, prevY = 0, decimals = properties.yaxisScaleDecimals;
        //
        // Work out the max value
        //
        if (properties.yaxisScaleMax) this.scale2 = RGraph.getScale({
            object: this,
            options: {
            ...
            if (typeof errorbars === "number") {
                ...
                for(var i1 = 0; i1 < this.data.length; ++i1){
                    if (typeof this.data[i1] === "number") properties.errorbars.push([
                        value,
                        null
                    ]);
                    else if (typeof this.data[i1] === "object" && !RGraph.isNull(this.data[i1])) for(var j = 0; j < this.data[i1].length; ++j)properties.errorbars.push([
                        value,
                        null
                    ]);
                }
            ...
            for(i1 = 0; i1 < this.data.length; ++i1){
                if (typeof this.data[i1] == "object") var value = properties.grouping === "grouped" ? Number(RGraph.arrayMax(this.data[i1], true)) : Number(RGraph.arraySum(this.data[i1]));
                else var value = Number(this.data[i1]);
                this.max = Math.max(Math.abs(this.max), Math.abs(value) + Number(typeof properties.errorbars === "object" && typeof properties.errorbars[i1] === "object" && !RGraph.isNull(properties.errorbars[i1]) && typeof properties.errorbars[i1][0] === "number" ? properties.errorbars[i1][0] : 0));
            }
        ...
        //
        if (variant === "3d") RGraph.draw3DAxes(this);
        //
        // Get the variant once, and draw the bars, be they regular, stacked or grouped
        //
        // Get these variables outside of the loop
        var xaxispos = properties.xaxisPosition, width = (this.canvas.width - this.marginLeft - this.marginRight) / this.data.length, orig_height = height, hmargin = properties.marginInner, shadow = properties.shadow, shadowColor = properties.shadowColor, shadowBlur = properties.shadowBlur, shadowOffsetX = properties.shadowOffsetx, shadowOffsetY = properties.shadowOffsety, strokeStyle = properties.colorsStroke, colors1 = properties.colors, sequentialColorIndex = 0;
        var height;
        //
        // Loop through the data
        //
        for(i1 = 0, len = this.data.length; i1 < len; i1 += 1){
            // Work out the height
            //The width is up outside the loop
            if (RGraph.arraySum(this.data[i1]) < 0) var height = (RGraph.arraySum(this.data[i1]) + this.scale2.min) / (this.scale2.max - this.scale2.min);
            else var height = (RGraph.arraySum(this.data[i1]) - this.scale2.min) / (this.scale2.max - this.scale2.min);
            height *= Math.abs(this.getYCoord(this.scale2.max) - this.getYCoord(this.scale2.min));
            var x = i1 * width + this.marginLeft;
            var y = xaxispos == "center" ? (this.canvas.height - this.marginTop - this.marginBottom) / 2 + this.marginTop - height : this.canvas.height - height - this.marginBottom;
            // xaxispos is top
            ...
            if (typeof this.data[i1] == "number") {
                // If the Y axis is offset change the bar start (the top of the bar)
                if (xaxispos === "bottom" && properties.yaxisScaleMin < 0) {
                    if (this.data[i1] >= 0) height = Math.abs(this.getYCoord(0) - this.getYCoord(this.data[i1]));
                    else {
                        y = this.getYCoord(0);
                        height = Math.abs(this.getYCoord(0) - this.getYCoord(this.data[i1]));
                    }
                }
                ...
                //
                if (barWidth < 0) alert("[RGRAPH] Warning: you have a negative bar width. This may be caused by the marginInner being too high or the width of the canvas not being sufficient.");
                // Set the fill color
                this.context.strokeStyle = strokeStyle;
                this.context.fillStyle = colors1[0];
                //
                // Sequential colors
                //
                if (properties.colorsSequential) this.context.fillStyle = colors1[i1];
                if (variant == "sketch") {
                    this.context.lineCap = "round";
                    var sketchOffset = 3;
                    this.context.beginPath();
                    this.context.strokeStyle = colors1[0];
                    //
                    // Sequential colors
                    //
                    if (properties.colorsSequential) this.context.strokeStyle = colors1[i1];
                    // Left side
                    this.context.moveTo(x + hmargin + 2, y + height - 2);
                    this.context.lineTo(x + hmargin - 1, y - 4);
                    // The top
                    this.context.moveTo(x + hmargin - 3, y + -2 + (this.data[i1] < 0 ? height : 0));
                    this.context.quadraticCurveTo(x + hmargin + (width - hmargin - hmargin) / 4, y + 0 + (this.data[i1] < 0 ? height : 0) + (this.data[i1] > 0 ? 10 : -10), x + hmargin + width + -1 - hmargin - hmargin, y + 0 + (this.data[i1] < 0 ? height : 0));
                    // The right side
                    ...
                    if (properties.variantSketchVerticals) for(var r = 0.2; r <= 0.8; r += 0.2){
                ...
                // Regular bar
                    ...
                    if (variant == "glass") {
                        RGraph.roundedRect({
                            ...
                            height: height,
                            radius: 5,
                            roundtl: this.data[i1] > 0,
                            roundtr: this.data[i1] > 0,
                            roundbl: this.data[i1] < 0,
                            roundbr: this.data[i1] < 0
                        });
                        this.context.stroke();
                        this.context.fill();
                    } else // On 9th April 2013 these two were swapped around so that the stroke happens SECOND so that any
                    ...
                    if (properties.corners === "round") {
                        ...
                        this.data[i1] < 0 || properties.xaxisPosition === "top" ? this.roundedCornersRectNegative(x + hmargin, y, barWidth, height) : this.roundedCornersRect(x + hmargin, y, barWidth, height);
                        this.context.stroke();
                        this.context.fill();
                    } else {
                        if (properties.xaxisPosition === "top") y += height;
                        this.context.beginPath();
                    ...
                    if (variant == "3d") {
                        ...
                        this.context.lineTo(x + hmargin + barWidth + properties.variantThreedOffsetx, this.data[i1] < 0 && xaxispos === "bottom" ? this.getYCoord(0) : this.data[i1] < 0 && y - properties.variantThreedOffsety < this.marginTop + this.halfgrapharea ? this.marginTop + this.halfgrapharea : y - properties.variantThreedOffsety);
                        this.context.lineTo(x + hmargin + barWidth + properties.variantThreedOffsetx, this.data[i1] < 0 && y - properties.variantThreedOffsety + height < this.marginTop + this.getYCoord(0) ? this.getYCoord(this.data[i1]) - properties.variantThreedOffsety : this.data[i1] > 0 ? y - properties.variantThreedOffsety + height : Math.min(y - properties.variantThreedOffsety + height, this.canvas.height - this.marginBottom));
                        ...
                        this.context.lineTo(x + hmargin + barWidth + properties.variantThreedOffsetx, this.data[i1] < 0 && xaxispos === "bottom" ? this.getYCoord(0) : this.data[i1] < 0 && y - properties.variantThreedOffsety < this.marginTop + this.halfgrapharea ? this.marginTop + this.halfgrapharea : y - properties.variantThreedOffsety);
                        // BR
                        this.context.lineTo(x + hmargin + barWidth + properties.variantThreedOffsetx, this.data[i1] < 0 && y - properties.variantThreedOffsety + height < this.getYCoord(0) ? this.getYCoord(0) : this.data[i1] > 0 ? y - properties.variantThreedOffsety + height : Math.min(y - properties.variantThreedOffsety + height, this.canvas.height - this.marginBottom));
                        // BL
                        this.context.lineTo(x + hmargin + barWidth, y + height);
                        this.context.lineTo(x + hmargin + barWidth, y);
                        this.context.closePath();
                    ...
                    // Glass variant
                        ...
                        this.context.rect(x + hmargin + 3, y + (this.data[i1] > 0 ? 3 : 0), barWidth / 2 - 2, height - 2);
                        this.context.fill();
                    }
                // Dot chart
                } else if (variant == "dot") {
                    ...
                    this.context.stroke();
                    this.context.beginPath();
                    this.context.fillStyle = this.properties.colors[i1];
                    this.context.arc(x + width / 2, y + (this.data[i1] > 0 ? 0 : height), 2, 0, 6.28, 0);
                ...
                } else alert("[BAR] Warning! Unknown variant: " + variant);
                this.coords.push([
                    x + hmargin,
                    y,
                    width - 2 * hmargin,
                ...
                if (typeof this.coords2[i1] == "undefined") this.coords2[i1] = [];
                this.coords2[i1].push([
                    x + hmargin,
                    y,
                    width - 2 * hmargin,
                    height
            ...
            // Stacked bar
            //
            } else if (this.data[i1] && typeof this.data[i1] == "object" && properties.grouping == "stacked") {
                if (this.scale2.min) alert("[ERROR] Stacked Bar charts with a Y min are not supported");
                var barWidth = width - 2 * hmargin;
                var redrawCoords = []; // Necessary to draw if the shadow is enabled
                var startY = 0;
                var dataset = this.data[i1];
                //
                // Check for a negative bar width
                //
                if (barWidth < 0) alert("[RGRAPH] Warning: you have a negative bar width. This may be caused by the marginInner being too high or the width of the canvas not being sufficient.");
                for(j = 0; j < dataset.length; ++j){
                    ...
                    //
                    this.context.strokeStyle = strokeStyle;
                    this.context.fillStyle = colors1[j];
                    if (properties.colorsReverse) this.context.fillStyle = colors1[this.data[i1].length - j - 1];
                    if (properties.colorsSequential && colors1[sequentialColorIndex]) this.context.fillStyle = colors1[sequentialColorIndex++];
                    else if (properties.colorsSequential) this.context.fillStyle = colors1[sequentialColorIndex - 1];
                    ...
                    if (height > 0) {
                        ...
                        if (j === 0 && properties.corners === "round" && properties.xaxisPosition === "bottom") {
                            ...
                            this.context.stroke();
                            this.context.fill();
                        } else this.path("b lj % lc % r % % % % s % f %", "miter", "square", x + hmargin, y, width - 2 * hmargin, height, this.context.strokeStyle, this.context.fillStyle);
                    }
                    if (j == 0) {
                        var startY = y;
                        var startX = x;
            ...
            // Grouped bar
            //
            } else if (this.data[i1] && typeof this.data[i1] == "object" && properties.grouping == "grouped") {
                var redrawCoords = [];
                this.context.lineWidth = properties.linewidth;
                for(j = 0; j < this.data[i1].length; ++j){
                    // Set the fill and stroke colors
                    this.context.strokeStyle = strokeStyle;
                    this.context.fillStyle = colors1[j];
                    //
                    ...
                    var individualBarWidth = (width - 2 * hmargin) / this.data[i1].length;
                    var height = (this.data[i1][j] + (this.data[i1][j] < 0 ? this.scale2.min : -1 * this.scale2.min)) / (this.scale2.max - this.scale2.min) * (this.canvas.height - this.marginTop - this.marginBottom);
                    var groupedMargin = properties.marginInnerGrouped;
                    var startX = x + hmargin + j * individualBarWidth;
                    //
                    // Check for a negative bar width
                    //
                    if (individualBarWidth < 0) alert("[RGRAPH] Warning: you have a negative bar width. This may be caused by the marginInner being too high or the width of the canvas not being sufficient.");
                    // If the X axis pos is in the center, we need to half the  height
                    if (xaxispos == "center") height /= 2;
                    //
                    ...
                    } else if (xaxispos == "center") var startY = this.marginTop + this.grapharea / 2 - height;
                    else {
                        var startY = this.getYCoord(0); //this.canvas.height - this.marginBottom - height;
                        var height = Math.abs(Math.abs(this.getYCoord(this.data[i1][j])) - this.getYCoord(0));
                        if (this.data[i1][j] >= 0) startY -= height;
                    }
                    if (properties.corners === "round") {
                        this.context.beginPath();
                        this.context.lineCap = "miter";
                        this.context.lineJoin = "square";
                        this.data[i1][j] < 0 ? this.roundedCornersRectNegative(startX + groupedMargin, startY, individualBarWidth - 2 * groupedMargin, height) : this.roundedCornersRect(startX + groupedMargin, startY, individualBarWidth - 2 * groupedMargin, height);
                        this.context.stroke();
                        this.context.fill();
                    } else {
                        this.context.beginPath();
                    ...
                    if (variant == "3d") {
                        ...
                        this.context.lineTo(startX + individualBarWidth - hmarginGrouped + properties.variantThreedOffsetx, this.data[i1][j] < 0 ? this.getYCoord(0) + Math.abs(height) - properties.variantThreedOffsety - (properties.xaxisPosition === "center" ? 0 : Math.abs(height) - this.properties.variantThreedOffsety) : this.getYCoord(0) - height - properties.variantThreedOffsety);
                        this.context.lineTo(startX + individualBarWidth - hmarginGrouped + properties.variantThreedOffsetx, this.data[i1][j] < 0 && startY + height - properties.variantThreedOffsety < this.getYCoord(0) ? this.getYCoord(0) : startY + height - properties.variantThreedOffsety);
                        ...
                        this.context.lineTo(startX + individualBarWidth + properties.variantThreedOffsetx - hmarginGrouped, this.data[i1][j] < 0 ? this.getYCoord(0) + Math.abs(height) - properties.variantThreedOffsety - (properties.xaxisPosition === "center" ? 0 : Math.abs(height) - this.properties.variantThreedOffsety) : this.getYCoord(0) - height - properties.variantThreedOffsety);
                        this.context.lineTo(startX + individualBarWidth + properties.variantThreedOffsetx - hmarginGrouped, +this.data[i1][j] < 0 && startY + height - 5 < this.getYCoord(0) ? height > this.properties.variantThreedOffsety ? this.getYCoord(0) + height - this.properties.variantThreedOffsety : this.getYCoord(0) : startY + height - properties.variantThreedOffsety);
                        // TL corner
                    ...
                    if (properties.shadow) redrawCoords.push([
                        startX + groupedMargin,
                        startY,
                        individualBarWidth - 2 * groupedMargin,
                        height,
    ...
    this.getShape = function(e) {
        ...
        var mouseXY = RGraph.getMouseXY(e), mouseX = mouseXY[0], mouseY = mouseXY[1], canvas = obj.canvas, context = obj.context, coords1 = obj.coords;
        for(var i1 = 0, len1 = coords1.length; i1 < len1; i1 += 1){
            if (obj.coords[i1].length == 0) continue;
            var left = coords1[i1][0], top = coords1[i1][1], width = coords1[i1][2], height = coords1[i1][3];
            // Old way of testing
            //if (mouseX >= left && mouseX <= (left + width) && mouseY >= top && mouseY <= (top + height)) {
            // Recreate the path/rectangle so that it can be tested
            //  ** DO NOT STROKE OR FILL IT **
            if (properties.tooltipsHotspotXonly) this.path("b r % % % %", left, this.marginTop, width, this.canvas.height - this.marginBottom);
            else {
                var indexes = RGraph.sequentialIndexToGrouped(i1, this.data);
                ...
                } else this.path("b r % % % %", left, top, width, height);
            }
            if (this.context.isPointInPath(mouseX, mouseY)) {
                if (properties.tooltips && RGraph.parseTooltipText) var tooltip = RGraph.parseTooltipText(properties.tooltips, i1);
                // Work out the dataset
                var dataset = 0, idx = i1;
                while(idx >= (typeof obj.data[dataset] === "object" && obj.data[dataset] ? obj.data[dataset].length : 1)){
                    if (typeof obj.data[dataset] === "number") idx -= 1;
                    else if (obj.data[dataset]) idx -= obj.data[dataset].length;
                    else idx -= 1;
                    dataset++;
                }
    ...
    this.getShapeByX = function(e) {
        ...
        //
        for(var i1 = 0, len1 = obj.coords.length; i1 < len1; i1++){
            if (obj.coords[i1].length == 0) continue;
            var mouseX = mouseCoords[0];
            var mouseY = mouseCoords[1];
            var left = obj.coords[i1][0];
            var top = obj.coords[i1][1];
            var width = obj.coords[i1][2];
            var height = obj.coords[i1][3];
            if (mouseX >= left && mouseX <= left + width) {
                if (properties.tooltips) var tooltip = RGraph.parseTooltipText ? RGraph.parseTooltipText(properties.tooltips, i1) : properties.tooltips[i1];
                var indexes = RGraph.sequentialIndexToGrouped(i1, this.data);
                return {
                    object: obj,
                    x: left,
    ...
    this.getValue = function(arg) {
        ...
        if (properties.xaxisPosition == "center") {
            ...
            value *= 2;
            if (value >= 0) value += this.scale2.min;
            else value -= this.scale2.min;
    ...
    this.getYCoord = function(value) {
        ...
        if (xaxispos == "top") {
            ...
            if (value < this.scale2.min) value = this.scale2.min;
    ...
    this.highlight = function(shape) {
        if (typeof properties.highlightStyle === "function") properties.highlightStyle(shape);
        else if (typeof properties.highlightStyle === "string" && properties.highlightStyle === "invert") {
            for(var i1 = 0; i1 < this.coords.length; ++i1)if (i1 !== shape.sequentialIndex) this.path("b r % % % % s % f %", this.coords[i1][0], this.coords[i1][1], this.coords[i1][2], this.coords[i1][3], properties.highlightStroke, properties.highlightFill);
        } else if (properties.grouping === "stacked" && shape.index === 0 && properties.xaxisPosition === "bottom") {
            this.context.beginPath();
            this.context.strokeStyle = properties.highlightStroke;
            this.context.fillStyle = properties.highlightFill;
            if (properties.corners === "round") this.roundedCornersRect(shape.x, shape.y, shape.width, shape.height);
            else this.context.rect(shape.x, shape.y, shape.width, shape.height);
            this.context.stroke();
            this.context.fill();
        } else // Add the new highlight
    ...
    this.getObjectByXY = function(e) {
        ...
        if (mouseXY[0] >= properties.marginLeft && mouseXY[0] <= this.canvas.width - properties.marginRight && mouseXY[1] >= properties.marginTop && mouseXY[1] <= this.canvas.height - properties.marginBottom) return this;
    };
    //
        ...
        if (properties.adjustable && RGraph.Registry.get("adjusting") && RGraph.Registry.get("adjusting").uid == this.uid) {
            ...
            if (shape) {
                ...
                if (this.stackedOrGrouped && properties.grouping == "grouped") {
                    ...
                    if (typeof this.data[indexes[0]] == "number") this.data[indexes[0]] = Number(value);
                    else if (!RGraph.isNull(this.data[indexes[0]])) this.data[indexes[0]][indexes[1]] = Number(value);
                } else if (typeof this.data[shape.sequentialIndex] == "number") this.data[shape.sequentialIndex] = Number(value);
    ...
    this.parseColors = function() {
        ...
        // colors
        var colors1 = properties.colors;
        if (colors1) for(var i1 = 0; i1 < colors1.length; ++i1)colors1[i1] = this.parseSingleColorForGradient(colors1[i1]);
        // keyColors
        var colors1 = properties.keyColors;
        if (colors1) for(var i1 = 0; i1 < colors1.length; ++i1)colors1[i1] = this.parseSingleColorForGradient(colors1[i1]);
    ...
    this.parseSingleColorForGradient = function(color) {
        ...
        if (color.match(/^gradient\((.*)\)$/i)) {
            ...
            if (color.match(/^gradient\(({.*})\)$/i)) return RGraph.parseJSONGradient({
                object: this,
                def: RegExp.$1
            });
            ...
            for(var j = 1, len1 = parts.length; j < len1; ++j)grad.addColorStop(j * diff, RGraph.trim(parts[j]));
        }
        return grad ? grad : color;
    };
    this.drawBevel = function() {
        var coords1 = this.coords, coords2 = this.coords2;
        if (properties.grouping == "stacked") {
            for(var i1 = 0; i1 < coords2.length; ++i1)if (coords2[i1] && coords2[i1][0] && coords2[i1][0][0]) {
                var x = coords2[i1][0][0];
                var y = coords2[i1][0][1];
                var w = coords2[i1][0][2];
                var arr = [];
                for(var j = 0; j < coords2[i1].length; ++j)arr.push(coords2[i1][j][3]);
            ...
            for(var i1 = 0; i1 < coords1.length; ++i1)if (coords1[i1]) {
                var x = coords1[i1][0];
                var y = coords1[i1][1];
                var w = coords1[i1][2];
                var h = coords1[i1][3];
    ...
    this.interactiveKeyHighlight = function(index) {
        ...
        this.coords2.forEach(function(value, idx, arr) {
            if (typeof value[index] == "object" && value[index]) {
                var x = value[index][0] - 0.5, y = value[index][1] - 0.5, w = value[index][2] + 1, h = value[index][3] + 1;
    ...
    this.drawLabelsAbove = this.drawAboveLabels = function() {
        ...
        if (labels && grouping === "grouped") for(var i1 = 0, len1 = data.length, sequentialIndex = 0; i1 < len1; i1 += 1){
            // Alignment for regular, positive bars
            if (typeof data[i1] === "number" && data[i1] >= 0) {
                var angle = angle;
                ...
                RGraph.text({
                    ...
                    x: coords2[i1][0][0] + coords2[i1][0][2] / 2 + offsetx,
                    y: coords2[i1][0][1] - offsety,
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals)),
                        value: Number(Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals)),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
            ...
            // Alignment for regular, negative bars
            } else if (typeof data[i1] === "number" && data[i1] < 0) {
                var angle = angle;
                ...
                RGraph.text({
                    ...
                    italic: textConf.italic,
                    x: coords2[i1][0][0] + coords2[i1][0][2] / 2 + offsetx,
                    y: coords2[i1][0][1] + coords2[i1][0][3] + offsety,
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals)),
                        value: Number(Number(typeof data[i1] === "object" ? data[i1][0] : data[i1]).toFixed(decimals)),
            ...
            // Alignment for grouped bars
            } else if (typeof data[i1] === "object") for(var j = 0, len2 = data[i1].length; j < len2; j += 1){
                var angle = angle;
                var halign = data[i1][j] < 0 ? "right" : "left";
                halign = angle === 0 ? "center" : halign;
                var valign = data[i1][j] < 0 ? "top" : "bottom";
                ...
                RGraph.text({
                    ...
                    bold: textConf.bold,
                    italic: textConf.italic,
                    x: coords2[i1][j][0] + coords2[i1][j][2] / 2 + offsetx,
                    y: properties.xaxisPosition === "top" ? coords2[i1][j][1] + coords2[i1][j][3] + 5 : coords2[i1][j][1] + (data[i1][j] < 0 ? coords2[i1][j][3] + offsety : -offsety),
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(Number(data[i1][j]).toFixed(decimals)),
                        value: Number(Number(data[i1][j]).toFixed(decimals)),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
            ...
            for(var i1 = 0, len1 = data.length, sequentialIndex = 0; i1 < len1; i1 += 1)if (typeof data[i1] === "object") {
                var angle = angle;
                ...
                RGraph.text({
                    ...
                    italic: textConf.italic,
                    x: coords2[i1][0][0] + coords2[i1][0][2] / 2 + offsetx,
                    y: coords2[i1][0][1] + (data[i1][0] < 0 ? coords2[i1][0][3] : 0) - offsety,
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(Number(RGraph.arraySum(data[i1])).toFixed(decimals)),
                        value: Number(Number(RGraph.arraySum(data[i1])).toFixed(decimals)),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
                ...
                sequentialIndex += data[i1].length;
            //
            ...
            //
                ...
                RGraph.text({
                    ...
                    text: specific ? specific[sequentialIndex] || "" : RGraph.numberFormat({
                        object: this,
                        number: Number(Number(data[i1]).toFixed(decimals)),
                        value: Number(Number(data[i1]).toFixed(decimals)),
                        unitspre: unitsPre,
                        unitspost: unitsPost,
                        point: point,
                        thousand: thousand,
    ...
    this.wave = function() {
        ...
        var obj = this, opt = arguments[0] || {}, labelsAbove = this.get("labelsAbove");
        opt.frames = opt.frames || 60;
        opt.startFrames = [];
        opt.counters = [];
        var framesperbar = opt.frames / 3, frame = -1, callback = arguments[1] || function() {}, original = RGraph.arrayClone(this.original_data);
        //
        // turn off the labelsAbove option whilst animating
        //
        this.set("labelsAbove", false);
        for(var i1 = 0, len1 = obj.data.length; i1 < len1; i1 += 1){
            opt.startFrames[i1] = opt.frames / 2 / (obj.data.length - 1) * i1;
            if (typeof obj.data[i1] === "object" && obj.data[i1]) {
                opt.counters[i1] = [];
                for(var j = 0; j < obj.data[i1].length; j++)opt.counters[i1][j] = 0;
            } else opt.counters[i1] = 0;
        }
        //
        ...
        function iterator() {
            ++frame;
            for(var i1 = 0, len1 = obj.data.length; i1 < len1; i1 += 1)if (frame > opt.startFrames[i1]) {
                if (typeof obj.data[i1] === "number") {
                    obj.data[i1] = Math.min(Math.abs(original[i1]), Math.abs(original[i1] * (opt.counters[i1]++ / framesperbar)));
                    // Make the number negative if the original was
                    if (original[i1] < 0) obj.data[i1] *= -1;
                } else if (!RGraph.isNull(obj.data[i1])) for(var j = 0, len2 = obj.data[i1].length; j < len2; j += 1){
                    obj.data[i1][j] = Math.min(Math.abs(original[i1][j]), Math.abs(original[i1][j] * (opt.counters[i1][j]++ / framesperbar)));
                    // Make the number negative if the original was
                    if (original[i1][j] < 0) obj.data[i1][j] *= -1;
                }
            } else obj.data[i1] = typeof obj.data[i1] === "object" && obj.data[i1] ? RGraph.arrayPad([], obj.data[i1].length, 0) : RGraph.isNull(obj.data[i1]) ? null : 0;
            if (frame >= opt.frames) {
                if (labelsAbove) {
    ...
    this.colorwave = this.colorWave = function() {
        ...
        opt.frames = opt.frames || 60;
        opt.startFrames = [];
        opt.counters = [], colors = obj.properties.colors;
        ...
        var framesperbar = opt.frames / 2, frame = -1, callback = arguments[1] || function() {}, originalColors = RGraph.arrayClone(obj.properties.colors);
        for(var i1 = 0, len1 = originalColors.length; i1 < len1; i1 += 1){
            opt.startFrames[i1] = opt.frames / 2 / (originalColors.length - 1) * i1;
            opt.counters[i1] = 0;
        }
        function iterator() {
            ++frame;
            for(var i1 = 0, len1 = colors.length; i1 < len1; i1 += 1)if (frame > opt.startFrames[i1] && colors[i1].match(/^rgba?\(([0-9 ]+),([0-9 ]+),([0-9 ]+)(,([ 0-9.]+)?)\)/)) // DO NOT USE SPACES!
            colors[i1] = "rgba({1},{2},{3},{4})".format(RegExp.$1, RegExp.$2, RegExp.$3, (frame - opt.startFrames[i1]) / framesperbar);
            else colors[i1] = colors[i1].replace(/,[0-9. ]+\)/, ",0)");
            if (frame >= opt.frames) callback(obj);
            else {
                RGraph.redrawCanvas(obj.canvas);
                RGraph.Effects.updateCanvas(iterator);
            }
    ...
    this.grow = function() {
        ...
        if (RGraph.isArray(opt.data)) {
            ...
            for(var i1 = 0; i1 < opt.data.length; ++i1){
                if (typeof opt.data[i1] === "object") for(var j = 0; j < opt.data[i1].length; ++j){
                    if (typeof opt.data[i1][j] === "string" && opt.data[i1][j].match(/(\+|\-)([0-9]+)/)) {
                        if (RegExp.$1 === "+") opt.data[i1][j] = this.original_data[i1][j] + parseInt(RegExp.$2);
                        else opt.data[i1][j] = this.original_data[i1][j] - parseInt(RegExp.$2);
                    }
                    ymax = Math.max(ymax, opt.data[i1][j]);
                }
                else if (typeof opt.data[i1] === "string" && opt.data[i1].match(/(\+|\-)([0-9]+)/)) {
                    if (RegExp.$1 === "+") opt.data[i1] = this.original_data[i1] + parseInt(RegExp.$2);
                    else opt.data[i1] = this.original_data[i1] - parseInt(RegExp.$2);
                    ymax = Math.max(ymax, opt.data[i1]);
                } else ymax = Math.max(ymax, opt.data[i1]);
            }
            ...
            if (typeof properties.yaxisScaleMax !== "number") this.set("yaxisScaleMax", scale.max);
        }
        //
        // turn off the labelsAbove option whilst animating
        //
        ...
        if (properties.yaxisScaleMax == null) {
            var ymax = 0;
            for(var i1 = 0; i1 < this.data.length; ++i1){
                if (RGraph.isArray(this.data[i1]) && properties.grouping === "stacked") ymax = Math.max(ymax, Math.abs(RGraph.arraySum(this.data[i1])));
                else if (RGraph.isArray(this.data[i1]) && properties.grouping === "grouped") {
                    for(var j = 0, group = []; j < this.data[i1].length; j++)group.push(Math.abs(this.data[i1][j]));
                    ymax = Math.max(ymax, Math.abs(RGraph.arrayMax(group)));
                } else ymax = Math.max(ymax, Math.abs(this.data[i1]));
            }
            var scale = RGraph.getScale({
                object: this,
                options: {
        ...
        var iterator = function() {
            ...
            for(var j = 0, len1 = obj.original_data.length; j < len1; ++j){
                if (typeof obj.data[j] === "object" && !RGraph.isNull(obj.data[j])) for(var k1 = 0, len2 = obj.data[j].length; k1 < len2; ++k1){
                    if (obj.firstDraw || !opt.data) obj.data[j][k1] = easingMultiplier * obj.original_data[j][k1];
                    else if (opt.data && opt.data.length === obj.original_data.length) {
                        var diff = opt.data[j][k1] - obj.original_data[j][k1];
                        obj.data[j][k1] = easingMultiplier * diff + obj.original_data[j][k1];
                    }
                }
                else {
                    if (obj.firstDraw || !opt.data) obj.data[j] = easingMultiplier * obj.original_data[j];
                    else if (opt.data && opt.data.length === obj.original_data.length) {
            ...
            // Call the callback function
                ...
                if (RGraph.isArray(opt.data)) {
                    var linear_data = RGraph.arrayLinearize(data);
                    for(var i1 = 0; i1 < linear_data.length; ++i1)if (!obj["$" + i1]) obj["$" + i1] = {};
                }
                obj.data = data;
    ...
    this.drawErrorbars = function() {
        var coords1 = this.coords, color = properties.errorbarsColor || "black", default_halfwidth = Math.min(properties.errorbarsCappedWidth, coords1[0][2]) / 2, x = 0, errorbars = properties.errorbars, length = 0;
        ...
        for(var i1 = 0; i1 < coords1.length; ++i1){
            var barX = coords1[i1][0], barY = coords1[i1][1], barW = coords1[i1][2], barH = coords1[i1][3];
            // Get the grouped version of the index
            var groupedIndexes = RGraph.sequentialIndexToGrouped(i1, this.data);
            // Determine if this is 
            if (typeof this.data[groupedIndexes[0]] === "object" && !RGraph.isNull(this.data[groupedIndexes[0]])) var isGrouped = true, group = groupedIndexes[0], subgroup = groupedIndexes[1];
            // Default to black
            color = properties.errorbarsColor || "black";
            // Set the perbar linewidth if the fourth option in the array
            // is specified
            if (errorbars[i1] && typeof errorbars[i1][3] === "number") this.context.lineWidth = errorbars[i1][3];
            // Set the halfwidth
            var halfwidth = errorbars[i1] && typeof errorbars[i1][4] === "number" ? errorbars[i1][4] / 2 : default_halfwidth;
            if (!properties.errorbarsCapped) halfwidth = 0;
            // Calulate the pixel size
            if (typeof errorbars[i1] === "number") {
                length = Math.abs(this.getYCoord(errorbars[i1]) - this.getYCoord(0));
                if (length) this.path("b % % l % % l % % l % % s %", barX + barW / 2, typeof this.data[i1] === "number" && this.data[i1] < 0 || isGrouped && this.data[group][subgroup] < 0 ? barY + barH : barY, barX + barW / 2, typeof this.data[i1] === "number" && this.data[i1] < 0 || isGrouped && this.data[group][subgroup] < 0 ? barY + barH + length : barY - length, barX + barW / 2 - halfwidth, typeof this.data[i1] === "number" && this.data[i1] < 0 || isGrouped && this.data[group][subgroup] < 0 ? Math.round(barY + barH + length) : Math.round(barY - length), barX + barW / 2 + halfwidth, typeof this.data[i1] === "number" && this.data[i1] < 0 || isGrouped && this.data[group][subgroup] < 0 ? Math.round(barY + barH + length) : Math.round(barY - length), color);
            } else if (typeof errorbars[i1] === "object" && !RGraph.isNull(errorbars[i1])) {
                var positiveLength = Math.abs(this.getYCoord(errorbars[i1][0]) - this.getYCoord(0));
                // Color
                if (typeof errorbars[i1][1] === "string") color = errorbars[i1][1];
                else if (typeof errorbars[i1][2] === "string") color = errorbars[i1][2];
                // Cap width
                halfwidth = typeof errorbars[i1][4] === "number" ? errorbars[i1][4] / 2 : default_halfwidth;
                if (!properties.errorbarsCapped) halfwidth = 0;
                if (!RGraph.isNull(errorbars[i1][0])) this.path("b m % % l % % l % % l % % s %", barX + barW / 2, barY + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), barX + barW / 2, barY - positiveLength + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), barX + barW / 2 - halfwidth, Math.round(barY - positiveLength) + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), barX + barW / 2 + halfwidth, Math.round(barY - positiveLength) + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), color);
                if (typeof errorbars[i1][1] === "number") {
                    var negativeLength = Math.abs(this.getYCoord(errorbars[i1][1]) - this.getYCoord(0));
                    this.path("b m % % l % % l % % l % % s %", barX + barW / 2, barY + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), barX + barW / 2, barY + negativeLength + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), barX + barW / 2 - halfwidth, Math.round(coords1[i1][1] + negativeLength) + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), barX + barW / 2 + halfwidth, Math.round(barY + negativeLength) + (this.data[i1] < 0 ? barH : 0) + (isGrouped && this.data[group][subgroup] < 0 ? barH : 0), color);
                }
            }
            // Reset the perbar linewidth to the default if the fourth option
            // in the array was specified specified
            if (errorbars[i1] && typeof errorbars[i1][3] === "number") this.context.lineWidth = properties.errorbarsLinewidth;
        }
    };
    //
    ...
    this.tooltipsFormattedCustom = function(specific, index) {
        var label;
        if (this.stackedOrGrouped) label = !RGraph.isNull(properties.tooltipsFormattedKeyLabels) && typeof properties.tooltipsFormattedKeyLabels === "object" && properties.tooltipsFormattedKeyLabels[index] ? properties.tooltipsFormattedKeyLabels[index] : "";
        else label = !RGraph.isNull(properties.tooltipsFormattedKeyLabels) && typeof properties.tooltipsFormattedKeyLabels === "object" && properties.tooltipsFormattedKeyLabels[specific.index] ? properties.tooltipsFormattedKeyLabels[specific.index] : "";
        return {
            label: label
        };
    };
    ...
    this.positionTooltipStatic = function(args) {
        ...
        if (this.data_arr[index] < 0) args.tooltip.style.top = parseFloat(args.tooltip.style.top) + coords[3] / 2 + "px";
    ...
    this.roundedCornersRectNegative = function(x, y, width, height) {
        ...
        var radius = properties.cornersRoundRadius;
        radius = Math.min(Math.abs(width / 2), Math.abs(height / 2), Math.abs(radius));
        // Save the existing state of the canvas so that it can be restored later
        this.context.save();
        // Translate to the given X/Y coordinates
        this.context.translate(x, y);
...
RGraph.CombinedChart = function() {
    ...
    this.objects = [];
    var objects = [];
    if (RGraph.isArray(arguments[0])) objects = arguments[0];
    else for(var i1 = 0; i1 < arguments.length; i1 += 1)objects[i1] = arguments[i1];
    for(var i1 = 0; i1 < objects.length; ++i1){
        this.objects[i1] = objects[i1];
        //
        ...
        if (this.objects[i1].type == "line") {
            var obj = this.objects[i1];
            //
        ...
        // Resizing
        //
        if (this.objects[i1].get("resizable")) var resizable_object = obj;
    }
    //
    // Resizing
    //
...
RGraph.CombinedChart.prototype.draw = function() {
    if (RGraph.isArray(this.objects)) {
        for(var i = 0; i < this.objects.length; ++i)if (this.objects[i].properties["combinedEffect"]) {
            // The options must be given as a string because of the
            // RGraph configuration system
            var options = this.objects[i].properties["combinedEffectOptions"] ? eval("(" + this.objects[i].properties["combinedEffectOptions"] + ")") : null, callback = this.objects[i].properties["combinedEffectCallback"], func = this.objects[i].properties["combinedEffect"];
...
RGraph.SegmentedBar = function(conf) {
    ...
    this.properties = {};
    // Process the data
    this.config.data.forEach(function(v, k1, arr) {
        arr[k1] = RGraph.arrayPad([], v, 1);
    });
    // Set the options on the segmentedBar object
    for(i in this.config.options)if (typeof i === "string") this.properties[i] = this.config.options[i];
    ...
    this.foreground = new RGraph.Bar({
        ...
        for(var i1 = 0; i1 <= count; ++i1)obj.path("cr 0 % 1000 %", obj.getYCoord(i1 / count * seg.foreground.scale2.max) - lw / 2, lw);
    });
...
//# sourceMappingURL=index.a4f51345.js.map
