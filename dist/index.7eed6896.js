...
(function(win, doc, undefined) {
    ...
    RGraph.getArgs = function(args, names) {
        ...
        if (args && args[0] && args.length === 1 && typeof args[0][names[0]] !== "undefined") {
            for(var i1 = 0; i1 < names.length; ++i1)if (typeof args[0][names[i1]] === "undefined") args[0][names[i1]] = null;
            return args[0];
        } else for(var i1 in names){
            ret[names[i1]] = typeof args[count] === "undefined" ? null : args[count];
            count += 1;
        }
        return ret;
    };
    ...
    RGraph.getScale = function(args) {
        ...
        if (!max) {
            var max = 1;
            for(var i1 = 0; i1 < numlabels; ++i1){
                var label = (((max - min) / numlabels + min) * (i1 + 1)).toFixed(decimals);
        ...
        //
            ...
            for(var i1 = 0; i1 < arr.length; ++i1)if (max > arr[i1]) {
                i1--;
                break;
            }
            scale.max = arr[i1];
            scale.labels = [];
            scale.values = [];
            for(var j1 = 0; j1 < numlabels; ++j1){
                var value = ((arr[i1] - min) / numlabels * (j1 + 1) + min).toFixed(decimals);
            ...
            for(var i1 = 0; i1 < numlabels; ++i1){
                scale.labels.push(RGraph.numberFormat({
                    object: args.object,
                    number: ((i1 + 1) / numlabels * (topValue - min) + min).toFixed(decimals),
                    unitspre: units_pre,
                    unitspost: units_post,
                    point: point,
                    thousand: thousand,
                ...
                scale.values.push(((i1 + 1) / numlabels * (topValue - min) + min).toFixed(decimals));
            }
            ...
            for(var i1 = 0; i1 < numlabels; ++i1){
                scale.labels.push(RGraph.numberFormat({
                    object: args.object,
                    number: ((i1 + 1) / numlabels * (max - min) + min).toFixed(decimals),
                    unitspre: units_pre,
                    unitspost: units_post,
                    thousand: thousand,
                    point: point,
                ...
                scale.values.push(((i1 + 1) / numlabels * (max - min) + min).toFixed(decimals));
            }
            // ???
            scale.max = max;
        }
        ...
        //
        for(var i1 = 0; i1 < scale.values.length; ++i1)scale.values[i1] = parseFloat(scale.values[i1]);
        return scale;
    };
    //
    ...
    RGraph.parseJSONGradient = function(args) {
        var obj = args.object, def = args.def, context = args.object.context;
        // Evaluate the JSON
        def = eval("(" + def + ")");
        // Create a radial gradient
        if (typeof def.r1 === "number" && typeof def.r2 === "number") // Create the gradient
        var grad = context.createRadialGradient(def.x1, def.y1, def.r1, def.x2, def.y2, def.r2);
        else var grad = context.createLinearGradient(def.x1, def.y1, def.x2, def.y2);
        // Add the parts to the gradient
        var diff = 1 / (def.colors.length - 1);
        grad.addColorStop(0, RGraph.trim(def.colors[0]));
        for(var j = 1, len = def.colors.length; j < len; ++j)grad.addColorStop(j * diff, RGraph.trim(def.colors[j]));
        return grad;
    };
    //
    ...
    RGraph.arrayInvert = function() {
        ...
        for(var i1 = 0, len = args.array.length; i1 < len; ++i1)args.array[i1] = !args.array[i1];
        return args.array;
    };
    //
    ...
    RGraph.arrayTrim = function() {
        ...
        // Trim the start
        for(var i1 = 0; i1 < args.array.length; i1++){
            if (args.array[i1]) content = true;
            if (content) out.push(args.array[i1]);
        }
        // Reverse the array and trim the start again
        out = RGraph.arrayReverse(out);
        var out2 = [], content = false;
        for(var i1 = 0; i1 < out.length; i1++){
            if (out[i1]) content = true;
            if (content) out2.push(out[i1]);
        }
    ...
    RGraph.arrayClone = function() {
        var args = RGraph.getArgs(arguments, "array");
        if (args.array === null || typeof args.array !== "object") return args.array;
    ...
    RGraph.arrayMax = function() {
        ...
        var max = null;
        if (typeof args.array === "number") return args.array;
        if (RGraph.isNull(args.array)) return 0;
        for(var i1 = 0, len = args.array.length; i1 < len; ++i1)if (typeof args.array[i1] === "number" && !isNaN(args.array[i1])) {
            var val = args.ignore ? Math.abs(args.array[i1]) : args.array[i1];
            if (typeof max === "number") max = Math.max(max, val);
            else max = val;
        }
        return max;
    };
    ...
    RGraph.arrayMin = function(args) {
        var args = RGraph.getArgs(arguments, "array,ignore");
        var max = null, min = null, ma = Math;
        if (typeof args.array === "number") return args.array;
        if (RGraph.isNull(args.array)) return 0;
        for(var i1 = 0, len = args.array.length; i1 < len; ++i1)if (typeof args.array[i1] === "number") {
            var val = args.ignore ? Math.abs(args.array[i1]) : args.array[i1];
            if (typeof min === "number") min = Math.min(min, val);
            else min = val;
        }
        return min;
    };
    ...
    RGraph.arrayPad = function() {
        ...
        if (args.array.length < args.length) for(var i1 = args.array.length; i1 < args.length; i1 += 1)args.array[i1] = args.value;
        return args.array;
    };
    //
    ...
    RGraph.arraySum = function() {
        ...
        // Account for null
        if (RGraph.isNull(args.array)) return 0;
        var i1, sum, len = args.array.length;
        for(i1 = 0, sum = 0; i1 < len; sum += args.array[i1++] || 0);
        return sum;
    };
    //
    ...
    RGraph.arrayLinearize = function() {
        ...
        for(var i1 = 0, len = args.length; i1 < len; ++i1){
            if (typeof args[i1] === "object" && args[i1]) for(var j1 = 0, len2 = args[i1].length; j1 < len2; ++j1){
                var sub = RGraph.arrayLinearize(args[i1][j1]);
                for(var k = 0, len3 = sub.length; k < len3; ++k)arr.push(sub[k]);
            }
            else arr.push(args[i1]);
        }
        return arr;
    };
    //
    ...
    RGraph.arrayShift = function() {
        var args = RGraph.getArgs(arguments, "array");
        var ret = [];
        for(var i1 = 1, len = args.array.length; i1 < len; ++i1)ret.push(args.array[i1]);
        return ret;
    };
    //
    ...
    RGraph.arrayReverse = function() {
        ...
        var newarr = [];
        for(var i1 = args.array.length - 1; i1 >= 0; i1 -= 1)newarr.push(args.array[i1]);
        return newarr;
    };
    //
    ...
    RGraph.abs = function() {
        ...
        if (typeof args.value === "string") args.value = parseFloat(args.value) || 0;
        if (typeof args.value === "number") return Math.abs(args.value);
        if (typeof args.value === "object") {
            for(i in args.value)if (typeof args.value[i] === "string" || typeof args.value[i] === "number" || typeof args.value[i] === "object") args.value[i] = RGraph.abs(args.value[i]);
            return args.value;
        }
        return 0;
    };
    ...
    //
    RGraph.clear = RGraph.Clear = function(args) {
        ...
        //
        if (RGraph.text.domNodeCache && RGraph.text.domNodeCache[args.canvas.id]) for(var i1 in RGraph.text.domNodeCache[args.canvas.id]){
            var el = RGraph.text.domNodeCache[args.canvas.id][i1];
        ...
        if (!color || color && color === "rgba(0,0,0,0)" || color === "transparent") {
            context.clearRect(-100, -100, args.canvas.width + 200, args.canvas.height + 200);
            // Reset the globalCompositeOperation
            context.globalCompositeOperation = "source-over";
        } else if (color) obj.path("fs % fr -100 -100 % %", color, args.canvas.width + 200, args.canvas.height + 200);
        else obj.path("fs % fr -100 -100 % %", obj.get("clearto"), args.canvas.width + 200, args.canvas.height + 200);
    ...
    RGraph.drawTitle = function() {
        ...
        //
        if (keypos && keypos != "margin") var valign = "center";
        else if (!keypos) var valign = "center";
        else var valign = "bottom";
        // if titleVpos is a number, use that
        if (typeof properties.titleVpos === "number") {
            vpos = properties.titleVpos * marginTop;
            if (properties.xaxisPosition === "top") vpos = properties.titleVpos * marginBottom + marginTop + (canvas.height - marginTop - marginBottom);
        } else {
            vpos = marginTop - size - 5;
            if (properties.xaxisPosition === "top") vpos = canvas.height - marginBottom + size + 5;
        }
        ...
        //
        if (typeof properties.titleHalign === "string") halign = properties.titleHalign;
        //
        // Vertical alignment can now (Jan 2013) be specified
        //
        if (typeof properties.titleValign === "string") valign = properties.titleValign;
        textConf.color;
        var oldColor = context.fillStyle, newColor = textConf.color;
    ...
    RGraph.getMouseXY = function() {
        ...
        if (typeof args.event.offsetX === "number" && typeof args.event.offsetY === "number") {
            ...
            if (typeof el.offsetParent !== "undefined") do {
    ...
    RGraph.isFixed = function() {
        var args = RGraph.getArgs(arguments, "canvas");
        var i1 = 0;
        while(args.canvas && args.canvas.tagName.toLowerCase() != "body" && i1 < 99){
            if (args.canvas.style.position == "fixed") return args.canvas;
    ...
    RGraph.redraw = function() {
        ...
        if (typeof args.color === "object" && args.color && typeof args.color.toString === "function" && typeof args.color.toString().indexOf === "function" && args.color.toString().indexOf("HTMLCanvasElement") > -1) {
            var opt = {
                canvas: args.color
            };
            // Has a color been given as well?
            if (arguments[1]) opt.color = arguments[1];
            return RGraph.redrawCanvas(opt);
        }
        // Get all of the canvas tags on the page
        var tags = document.getElementsByTagName("canvas");
        for(var i1 = 0, len = tags.length; i1 < len; ++i1){
            if (tags[i1] && tags[i1].__object__ && tags[i1].__object__.isrgraph) // Only clear the canvas if it's not Trace'ing - this applies to the Line/Scatter Trace effects
            {
                if (!tags[i1].noclear) RGraph.clear(tags[i1], args.color ? args.color : null);
            }
        }
        // Go through the object registry and redraw *all* of the canvas'es that have been registered
        for(var i1 = 0, len = objectRegistry.length; i1 < len; ++i1)if (objectRegistry[i1]) {
            var id = objectRegistry[i1][0];
            objectRegistry[i1][1].draw();
        }
    };
    //
    ...
    RGraph.redrawCanvas = function() {
        ...
        //
        for(var i1 = 0, len = objects.length; i1 < len; ++i1){
            if (objects[i1]) {
                if (objects[i1] && objects[i1].isrgraph) objects[i1].draw();
            }
        }
    };
    //
    ...
    RGraph.Background.draw = function() {
        var args = RGraph.getArgs(arguments, "object");
        var properties = args.object.properties, height = 0, marginLeft = args.object.marginLeft, marginRight = args.object.marginRight, marginTop = args.object.marginTop, marginBottom = args.object.marginBottom, variant = properties.variant;
        ...
        if (args.object.type !== "bar" && args.object.type !== "waterfall" && args.object.type !== "hbar" && args.object.type !== "line" && args.object.type !== "gantt" && args.object.type !== "scatter" && typeof properties.xaxisTitle === "string" && properties.xaxisTitle.length) {
            ...
            if (typeof properties.xaxisTitlePos === "number") vpos = args.object.canvas.height - marginBottom * properties.xaxisTitlePos;
        ...
        // Y axis title
        if (args.object.type !== "bar" && args.object.type !== "waterfall" && args.object.type !== "hbar" && args.object.type !== "line" && args.object.type !== "gantt" && args.object.type !== "scatter" && typeof properties.yaxisTitle === "string" && properties.yaxisTitle.length) {
            var size = properties.textSize + 2;
            var font = properties.textFont;
            ...
            if (typeof properties.yaxisTitlePos == "number") var yaxis_title_pos = properties.yaxisTitlePos * marginLeft;
            else if (args.object.type === "hbar" && RGraph.isNull(properties.yaxisTitlePos)) var yaxis_title_pos = properties.marginLeft - args.object.yaxisLabelsSize;
            else {
                if (args.object && args.object.scale2) var yaxisTitleDimensions = RGraph.measureText({
                ...
                else // This is here to allow for the drawing API background
                // object
                yaxisTitleDimensions = [
                    0,
                    0
                ];
            ...
            if (properties.yaxisTitleAlign == "right" || properties.yaxisTitlePosition == "right" || args.object.type === "hbar" && properties.yaxisPosition === "right" && typeof properties.yaxisTitleAlign === "undefined" && typeof properties.yaxisTitlePosition === "undefined") {
                angle = 90;
                yaxis_title_pos = typeof properties.yaxisTitlePos === "number" ? args.object.canvas.width - marginRight + properties.yaxisTitlePos * marginRight : args.object.canvas.width - marginRight + (properties.yaxisLabelsSize || properties.textSize) + 10;
            }
            var y = (args.object.canvas.height - marginTop - marginBottom) / 2 + marginTop;
            // Specifically specified X/Y positions
            if (typeof properties.yaxisTitleX === "number") yaxis_title_pos = properties.yaxisTitleX;
        ...
        if (typeof properties.backgroundBarsCount === "number") numbars = properties.backgroundBarsCount;
        ...
        for(var i1 = 0; i1 < numbars; i1 += 2)args.object.context.rect(marginLeft, i1 * barHeight + marginTop, args.object.canvas.width - marginLeft - marginRight, barHeight);
        args.object.context.fill();
        args.object.context.beginPath();
        args.object.context.fillStyle = properties.backgroundBarsColor2;
        args.object.context.strokeStyle = args.object.context.fillStyle;
        for(var i1 = 1; i1 < numbars; i1 += 2)args.object.context.rect(marginLeft, i1 * barHeight + marginTop, args.object.canvas.width - marginLeft - marginRight, barHeight);
        args.object.context.fill();
            ...
            if (properties.backgroundGrid) {
                ...
                if (properties.backgroundGridAutofit) {
                    ...
                    if (properties.backgroundGridAutofitAlign) {
                        ...
                        if (obj.type === "hbar") obj.set("backgroundGridHlinesCount", obj.data.length);
                        // Align the vertical lines for the line
                        if (obj.type === "line") {
                            if (typeof properties.backgroundGridVlinesCount === "number") ;
                            else if (properties.xaxisLabels && properties.xaxisLabels.length) obj.set("backgroundGridVlinesCount", properties.xaxisLabels.length - 1);
                            else obj.set("backgroundGridVlinesCount", obj.data[0].length > 0 ? obj.data[0].length - 1 : 0);
                        } else if (obj.type === "waterfall") obj.set("backgroundGridVlinesCount", obj.data.length + (properties.total ? 1 : 0));
                        else if (obj.type === "bar") // 13/12/2018
                        //
                        ...
                        obj.set("backgroundGridVlinesCount", obj.data.length);
                            ...
                            if (typeof properties.backgroundGridVlinesCount !== "number") {
                                ...
                                if (RGraph.isArray(properties.xaxisLabels) && properties.xaxisLabels.length) obj.set("backgroundGridVlinesCount", properties.xaxisLabels.length);
                                else obj.set("backgroundGridVlinesCount", 10);
                            }
                        } else if (obj.type === "gantt") {
                            if (typeof obj.get("backgroundGridVlinesCount") === "number") ;
                            else obj.set("backgroundGridVlinesCount", properties.xaxisScaleMax);
                            obj.set("backgroundGridHlinesCount", obj.data.length);
                        // HBar
                        } else if (obj.type === "hbar" && RGraph.isNull(properties.backgroundGridHlinesCount)) obj.set("backgroundGridHlinesCount", obj.data.length);
                    }
                ...
                if (properties.backgroundGridDashed && typeof cacheContext.setLineDash == "function") cacheContext.setLineDash([
                    3,
                    5
                ]);
                // Dotted background grid
                if (properties.backgroundGridDotted && typeof cacheContext.setLineDash == "function") cacheContext.setLineDash([
                    1,
                    3
                ]);
                obj.context.beginPath();
        ...
        RGraph.cachedDraw(args.object, args.object.uid + "_background", func);
        // If it's a bar and 3D variant, translate
        if (variant == "3d") args.object.context.restore();
        // Reset the line dash
        ...
        if (typeof args.object.properties.title == "string") RGraph.drawTitle(args.object, properties.title, args.object.marginTop, null, properties.titleSize ? properties.titleSize : properties.textSize + 2, args.object);
    };
    //
    ...
    RGraph.numberFormat = function(args) {
        ...
        RegExp.$1 = "";
        var i1, j1;
        if (typeof args.formatter === "function") return args.formatter(args);
        ...
        var foundPoint;
        for(i1 = args.number.length - 1, j1 = 0; i1 >= 0; j1++, i1--){
            var character = args.number.charAt(i1);
            if (j1 % 3 == 0 && j1 != 0) output += seperator;
            //
            // Build the output
            //
            output += character;
        ...
        var rev = output;
        output = "";
        for(i1 = rev.length - 1; i1 >= 0; i1--)output += rev.charAt(i1);
        // Tidy up
    ...
    RGraph.drawBars = function() {
        var args = RGraph.getArgs(arguments, "object"), properties = args.object.properties, hbars = properties.backgroundHbars;
        if (hbars === null) return;
        //
        // Draws a horizontal bar
        //
        args.object.context.beginPath();
        for(var i1 = 0, len = hbars.length; i1 < len; ++i1){
            var start = hbars[i1][0];
            var length = hbars[i1][1];
            var color = hbars[i1][2];
            ...
            if (properties.xaxisPosition == "center" && start == args.object.scale2.max && length < args.object.scale2.max * -2) length = args.object.scale2.max * -2;
            //
            // Draw the bar
            //
    ...
    RGraph.drawInGraphLabels = function() {
        var args = RGraph.getArgs(arguments, "object");
        var properties = args.object.properties, labels = properties.labelsIngraph, labels_processed = [];
        // Defaults
        var fgcolor = "black", bgcolor = "white", direction = 1;
        if (!labels) return;
        // Get the text configuration
        var textConf = RGraph.getTextConf({
            object: args.object,
        ...
        //
        for(var i1 = 0, len = labels.length; i1 < len; i1 += 1){
            if (typeof labels[i1] === "number") for(var j1 = 0; j1 < labels[i1]; ++j1)labels_processed.push(null);
            else if (typeof labels[i1] === "string" || typeof labels[i1] === "object") labels_processed.push(labels[i1]);
            else labels_processed.push("");
        }
        // Turn off any shadow
        RGraph.noShadow(args.object);
        if (labels_processed && labels_processed.length > 0) {
            for(var i1 = 0, len = labels_processed.length; i1 < len; i1 += 1)if (labels_processed[i1]) {
                var coords = args.object.coords[i1];
                if (coords && coords.length > 0) {
                    ...
                    var length = typeof labels_processed[i1][4] === "number" ? labels_processed[i1][4] : 25;
                    args.object.context.beginPath();
                    ...
                    if (args.object.type === "bar") {
                        ...
                        // X axis at the top
                        //
                        if (args.object.get("xaxisPosition") == "top") length *= -1;
                        if (properties.variant == "dot") {
                            args.object.context.moveTo(Math.round(x), args.object.coords[i1][1] - 5);
                            args.object.context.lineTo(Math.round(x), args.object.coords[i1][1] - 5 - length);
                            var text_x = Math.round(x);
                            var text_y = args.object.coords[i1][1] - 5 - length;
                        ...
                        if (typeof labels_processed[i1] == "object" && typeof labels_processed[i1][3] == "number" && labels_processed[i1][3] == -1) {
                            // Draw an up arrow
                            drawUpArrow(x, y);
                            var valign = "top";
                            var text_x = x;
                            ...
                            var text_x = x;
                            var text_y = y - 5 - length;
                            if (text_y < 5 && (typeof labels_processed[i1] === "string" || typeof labels_processed[i1][3] === "undefined")) {
                                text_y = y + 5 + length;
                                var valign = "top";
                            }
                            if (valign === "top") /// Draw an down arrow
                            drawUpArrow(x, y);
                            else /// Draw an up arrow
                            drawDownArrow(x, y);
                        }
                        args.object.context.fill();
                    }
                    args.object.context.beginPath();
                    // Foreground color
                    if (typeof labels_processed[i1] === "object" && typeof labels_processed[i1][1] === "string") args.object.context.fillStyle = labels_processed[i1][1];
                    else args.object.context.fillStyle = properties.labelsIngraphColor;
                    RGraph.text({
                        object: args.object,
                        font: textConf.font,
                        size: textConf.size,
                        ...
                        text: typeof labels_processed[i1] === "object" && typeof labels_processed[i1][0] === "string" ? labels_processed[i1][0] : labels_processed[i1],
                        valign: valign || "bottom",
                        halign: "center",
                        bounding: true,
                        "bounding.fill": typeof labels_processed[i1] === "object" && typeof labels_processed[i1][2] === "string" ? labels_processed[i1][2] : "white",
                        tag: "labels ingraph"
                    });
                    args.object.context.fill();
                }
    ...
    RGraph.hideCrosshairCoords = function() {
        ...
        if (div && div.style.opacity == 1 && div.__object__.get("crosshairsCoordsFadeout")) {
    ...
    RGraph.draw3DAxes = function() {
        ...
        var marginLeft = args.object.marginLeft, marginRight = args.object.marginRight, marginTop = args.object.marginTop, marginBottom = args.object.marginBottom, xaxispos = properties.xaxisPosition, graphArea = args.object.canvas.height - marginTop - marginBottom, halfGraphArea = graphArea / 2, offsetx = properties.variantThreedOffsetx, offsety = properties.variantThreedOffsety, xaxis = properties.variantThreedXaxis, yaxis = properties.variantThreedYaxis;
        //
        // Draw the 3D Y axis
        //
        ...
        // X axis
        if (xaxis) {
            if (xaxispos === "center") args.object.path("b m % % l % % l % % l % % c s #aaa f #ddd", marginLeft, marginTop + halfGraphArea, marginLeft + offsetx, marginTop + halfGraphArea - offsety, args.object.canvas.width - marginRight + offsetx, marginTop + halfGraphArea - offsety, args.object.canvas.width - marginRight, marginTop + halfGraphArea);
            else {
                if (args.object.type === "hbar") var xaxisYCoord = args.object.canvas.height - args.object.properties.marginBottom;
                else var xaxisYCoord = args.object.getYCoord(0);
                args.object.path("m % % l % % l % % l % % c s #aaa f #ddd", marginLeft, xaxisYCoord, marginLeft + offsetx, xaxisYCoord - offsety, args.object.canvas.width - marginRight + offsetx, xaxisYCoord - offsety, args.object.canvas.width - marginRight, xaxisYCoord);
            }
        }
    };
    //
    ...
    RGraph.draw3DYAxis = function(args) {
        ...
        if ((args.object.type === "hbar" || args.object.type === "bar") && properties.yaxisPosition === "center") var x = (args.object.canvas.width - marginLeft - marginRight) / 2 + marginLeft;
        else if ((args.object.type === "hbar" || args.object.type === "bar") && properties.yaxisPosition === "right") var x = args.object.canvas.width - marginRight;
        else var x = marginLeft;
        args.object.path("b m % % l % % l % % l % % s #aaa f #ddd", x, marginTop, x + offsetx, marginTop - offsety, x + offsetx, args.object.canvas.height - marginBottom - offsety, x, args.object.canvas.height - marginBottom);
    //}
    };
    //
    ...
    RGraph.roundedRect = function() {
        ...
        r = Math.min(Math.min(args.width, args.height) / 2, args.radius);
        // The corners
        var corner_tl = args.roundtl === false ? false : true, corner_tr = args.roundtr === false ? false : true, corner_bl = args.roundbl === false ? false : true, corner_br = args.roundbr === false ? false : true;
        args.context.beginPath();
        args.context.moveTo(args.x, args.y + r);
        // Top left corner
        if (corner_tl) args.context.arc(args.x + r, args.y + r, r, RGraph.PI, RGraph.PI + RGraph.HALFPI, false);
        else {
            args.context.lineTo(args.x, args.y);
            args.context.lineTo(args.x + r, args.y);
        }
        // Top right corner
        if (corner_tr) args.context.arc(args.x + args.width - r, args.y + r, r, RGraph.PI + RGraph.HALFPI, 0, false);
        else {
            args.context.lineTo(args.x + args.width, args.y);
            args.context.lineTo(args.x + args.width, args.y + r);
        }
        // Bottom right corner
        if (corner_br) args.context.arc(args.x + args.width - r, args.y + args.height - r, r, 0, RGraph.HALFPI, false);
        else {
            args.context.lineTo(args.x + args.width, args.y + args.height);
            args.context.lineTo(args.x + args.width - r, args.y + args.height);
        }
        // Bottom left corner
        if (corner_bl) args.context.arc(args.x + r, args.y - r + args.height, r, RGraph.HALFPI, RGraph.PI, false);
        else {
    ...
    RGraph.addCustomEventListener = function() {
        ...
        if (typeof RGraph.events[args.object.uid] === "undefined") RGraph.events[args.object.uid] = [];
    ...
    RGraph.fireCustomEvent = function() {
        ...
        if (args.object && args.object.isrgraph) {
            ...
            if (args.object[args.name]) args.object[args.name](args.object);
            var uid = args.object.uid;
            if (typeof uid === "string" && typeof RGraph.events === "object" && typeof RGraph.events[uid] === "object" && RGraph.events[uid].length > 0) {
                for(var j1 = 0; j1 < RGraph.events[uid].length; ++j1)if (RGraph.events[uid][j1] && RGraph.events[uid][j1][1] === args.name) RGraph.events[uid][j1][2](args.object);
            }
        }
    };
    //
    ...
    RGraph.removeAllCustomEventListeners = function() {
        ...
        if (args.uid && RGraph.events[args.uid]) RGraph.events[args.uid] = [];
        else {
            RGraph.events = [];
    ...
    RGraph.removeCustomEventListener = function() {
        ...
        if (typeof RGraph.events === "object" && typeof RGraph.events[args.object.uid] === "object" && typeof RGraph.events[args.object.uid][args.index] === "object") RGraph.events[args.object.uid][args.index] = null;
    };
    //
    ...
    RGraph.drawBackgroundImage = function(args) {
        ...
        if (typeof properties.backgroundImage === "string") {
            ...
            } else img = args.object.canvas.__rgraph_background_image__;
            ...
            if (typeof align === "string") {
                if (align.indexOf("right") != -1) var x = args.object.canvas.width - (properties.backgroundImageW || img.width) - marginRight;
                else var x = marginLeft;
                if (align.indexOf("bottom") != -1) var y = args.object.canvas.height - (properties.backgroundImageH || img.height) - marginBottom;
                else var y = marginTop;
            } else {
                var x = marginLeft || 25;
                var y = marginTop || 25;
            }
    ...
    RGraph.hasTooltips = function() {
        var args = RGraph.getArgs(arguments, "object");
        var properties = args.object.properties;
        if (typeof properties.tooltips == "object" && properties.tooltips) for(var i1 = 0, len = properties.tooltips.length; i1 < len; ++i1){
            if (!RGraph.isNull(args.object.get("tooltips")[i1])) return true;
        }
        else if (typeof properties.tooltips === "function") return true;
        return false;
    };
    //
    ...
    RGraph.ObjectRegistry.add = function() {
        ...
        RGraph.ObjectRegistry.objects.byUID[RGraph.ObjectRegistry.objects.byUID.length] = [
            uid,
            args.object
        ];
        //
    ...
    RGraph.ObjectRegistry.remove = function() {
        ...
        var uid = args.object.uid;
        for(var i1 = 0; i1 < RGraph.ObjectRegistry.objects.byUID.length; ++i1)if (RGraph.ObjectRegistry.objects.byUID[i1] && RGraph.ObjectRegistry.objects.byUID[i1][1].uid == uid) RGraph.ObjectRegistry.objects.byUID[i1] = null;
        for(var i1 = 0; i1 < RGraph.ObjectRegistry.objects.byCanvasID.length; ++i1)if (RGraph.ObjectRegistry.objects.byCanvasID[i1] && RGraph.ObjectRegistry.objects.byCanvasID[i1][1] && RGraph.ObjectRegistry.objects.byCanvasID[i1][1].uid == uid) RGraph.ObjectRegistry.objects.byCanvasID[i1] = null;
    };
    //
    ...
    RGraph.ObjectRegistry.clear = function() {
        if (typeof arguments[0] === "object" && !RGraph.isNull(arguments) && (typeof arguments[0].canvas === "object" || typeof arguments[0].id === "string")) var args = arguments[0];
        else {
            // A string has been given
            if (typeof arguments[0] === "string") var args = {
                id: arguments[0]
            };
            else if (typeof arguments[0] === "object" && arguments[0].getContext) var args = {
                canvas: arguments[0]
            };
            else {
                RGraph.ObjectRegistry.objects = {};
        ...
        if (args.id || args.canvas) {
            ...
            if (args.canvas) args.id = args.canvas.id;
            var objects = RGraph.ObjectRegistry.getObjectsByCanvasID(args.id);
            for(var i1 = 0, len = objects.length; i1 < len; ++i1)RGraph.ObjectRegistry.remove(objects[i1]);
    ...
    RGraph.ObjectRegistry.list = function() {
        ...
        var list = [];
        for(var i1 = 0, len = RGraph.ObjectRegistry.objects.byUID.length; i1 < len; ++i1)if (RGraph.ObjectRegistry.objects.byUID[i1]) list.push(RGraph.ObjectRegistry.objects.byUID[i1][1].type);
        if (args.alert) $p(list);
        else return list;
    };
    //
    ...
    RGraph.ObjectRegistry.clearByType = function() {
        ...
        for(var i1 = 0, len = objects.length; i1 < len; ++i1)if (objects[i1]) {
            var uid = objects[i1][0], obj = objects[i1][1];
            if (obj && obj.type === args.type) RGraph.ObjectRegistry.remove(obj);
        }
    };
    //
    ...
    RGraph.ObjectRegistry.iterate = function() {
        ...
        var objects = RGraph.ObjectRegistry.objects.byUID;
        for(var i1 = 0, len = objects.length; i1 < len; ++i1)if (typeof args.type === "string") {
            var types = args.type.split(/,/);
            for(var j1 = 0, len2 = types.length; j1 < len2; ++j1)if (types[j1] == objects[i1][1].type) args.func(objects[i1][1]);
        } else args.func(objects[i1][1]);
    };
    //
    ...
    RGraph.ObjectRegistry.getObjectsByCanvasID = function() {
        ...
        for(var i1 = 0, len = store.length; i1 < len; ++i1)if (store[i1] && store[i1][0] == args.id) ret.push(store[i1][1]);
        return ret;
    };
    //
    ...
    //
    RGraph.ObjectRegistry.firstbyxy = RGraph.ObjectRegistry.getObjectByXY = RGraph.ObjectRegistry.getFirstObjectByXY = function() {
        var args = RGraph.getArgs(arguments, "event");
        var canvas = args.event.target;
        var ret = null;
        var objects = RGraph.ObjectRegistry.getObjectsByCanvasID(canvas.id);
        for(var i1 = objects.length - 1; i1 >= 0; --i1){
            var obj = objects[i1].getObjectByXY(args.event);
            if (obj) return obj;
        }
    };
    //
    ...
    RGraph.ObjectRegistry.getObjectsByXY = function() {
        var args = RGraph.getArgs(arguments, "event");
        var canvas = args.event.target, ret = [], objects = RGraph.ObjectRegistry.getObjectsByCanvasID(canvas.id);
    ...
    //
    RGraph.ObjectRegistry.get = RGraph.ObjectRegistry.getObjectByUID = function() {
        var args = RGraph.getArgs(arguments, "uid");
        var objects = RGraph.ObjectRegistry.objects.byUID;
        for(var i1 = 0, len = objects.length; i1 < len; ++i1){
            if (objects[i1] && objects[i1][1].uid == args.uid) return objects[i1][1];
        }
    };
    //
    ...
    RGraph.ObjectRegistry.bringToFront = function() {
        ...
        RGraph.ObjectRegistry.add(args.object);
        if (redraw) RGraph.redrawCanvas(args.object.canvas);
    };
    //
    ...
    //
    RGraph.ObjectRegistry.type = RGraph.ObjectRegistry.getObjectsByType = function() {
        var args = RGraph.getArgs(arguments, "type");
        var objects = RGraph.ObjectRegistry.objects.byUID;
        var ret = [];
        for(i in objects)if (objects[i] && objects[i][1] && objects[i][1].type && objects[i][1].type && objects[i][1].type == args.type) ret.push(objects[i][1]);
        return ret;
    };
    //
    ...
    //
    RGraph.ObjectRegistry.first = RGraph.ObjectRegistry.getFirstObjectByType = function() {
        var args = RGraph.getArgs(arguments, "type");
        var objects = RGraph.ObjectRegistry.objects.byUID;
        for(var i1 in objects){
            if (objects[i1] && objects[i1][1] && objects[i1][1].type == args.type) return objects[i1][1];
        }
        return null;
    };
    //
    ...
    RGraph.getAngleByXY = function() {
        ...
        if (args.x >= args.cx && args.y >= args.cy) angle += RGraph.TWOPI;
        else if (args.x >= args.cx && args.y < args.cy) angle = RGraph.HALFPI - angle + (RGraph.PI + RGraph.HALFPI);
        else if (args.x < args.cx && args.y < args.cy) angle += RGraph.PI;
        else angle = RGraph.PI - angle;
        //
    ...
    RGraph.installEventListeners = function() {
        ...
        } else if (RGraph.hasTooltips(args.object) || properties.adjustable || properties.annotatable || properties.contextmenu || properties.keyInteractive || typeof args.object.onclick === "function" || typeof args.object.onmousemove === "function" || typeof args.object.onmouseout === "function" || typeof args.object.onmouseover === "function") alert("[RGRAPH] You appear to have used dynamic features but not included the file: RGraph.common.dynamic.js");
    };
    //
    ...
    RGraph.pr = function(obj) {
        ...
        switch(typeof obj){
            case "string":
                str += args.object + " (" + typeof args.object + ", " + args.object.length + ")";
                break;
            case "number":
                str += args.object + " (" + typeof args.object + ")";
                break;
            case "boolean":
                str += args.object + " (" + typeof args.object + ")";
                break;
            ...
            case "object":
                // In case of null
                if (RGraph.isNull(args.object)) str += "null";
                else {
                    str += "Object {\n";
                    for(var j1 in obj)str += indent + "    " + j1 + " => " + RGraph.pr(args.object[j1], true, indent + "    ", counter + 1) + "\n";
                    str += indent + "}";
                }
                break;
            default:
    ...
    RGraph.dashedLine = function() {
        var args = RGraph.getArgs(arguments, "context,x1,y1,x2,y2,size"), dx = args.x2 - args.x1, dy = args.y2 - args.y1, num = Math.floor(Math.sqrt(dx * dx + dy * dy) / (args.size || 3)), xLen = dx / num, yLen = dy / num, count = 0;
        do {
            if (count % 2 == 0 && count > 0) args.context.lineTo(args.x1, args.y1);
            else args.context.moveTo(args.x1, args.y1);
            args.x1 += xLen;
            args.y1 += yLen;
            count++;
        }while (count <= num);
    ...
    RGraph.AJAX = function() {
        ...
        if (httpRequest && httpRequest.setRequestHeader) httpRequest.setRequestHeader("Cache-Control", "no-cache");
        httpRequest.send();
    };
    //
    ...
    RGraph.AJAX.post = RGraph.AJAX.POST = function() {
        ...
        for(i in args.data)if (typeof i == "string") crumbs.push(i + "=" + encodeURIComponent(args.data[i]));
    ...
    RGraph.AJAX.getCSV = function() {
        ...
        RGraph.AJAX(args.url, function() {
            ...
            for(var i1 = 0, len = arr.length; i1 < len; ++i1)arr[i1] = parseFloat(arr[i1]);
            args.callback(arr);
        });
    };
    //
    ...
    RGraph.measureText = function() {
        ...
        if (typeof RGraph.measuretext_cache == "object" && RGraph.measuretext_cache[str]) return RGraph.measuretext_cache[str];
    ...
    RGraph.text = function(args) {
        ...
        if (arguments[0] && arguments[1] && (typeof arguments[1].text === "string" || typeof arguments[1].text === "number")) var obj = arguments[0], args = arguments[1];
        else {
            var obj = args.object;
            //
            // The text arg must be a string or a number
            //
            if (typeof args.text === "number") args.text = String(args.text);
        }
        // Get the defaults for the text function from RGraph.text.defaults object
        for(var i1 in RGraph.text.defaults)if (typeof i1 === "string" && typeof args[i1] === "undefined") args[i1] = RGraph.text.defaults[i1];
        ...
        function domtext() {
            ...
            if (!obj.canvas.rgraph_domtext_wrapper) {
                ...
                if ((obj.type === "bar" || obj.type === "bipolar" || obj.type === "hbar") && obj.properties.variant === "3d") wrapper.style.transform = "skewY(5.7deg)";
            ...
            } else wrapper = obj.canvas.rgraph_domtext_wrapper;
            var defaults = {
                size: 12,
                font: "Arial",
                italic: "normal",
            ...
            if (typeof RGraph.text.domNodeCache === "undefined") RGraph.text.domNodeCache = new Array();
            ...
            if (!RGraph.text.domNodeCache[obj.id] || !RGraph.text.domNodeCache[obj.id][cacheKey]) {
                ...
                span.className = " rgraph_accessible_text rgraph_accessible_text_" + obj.id + " rgraph_accessible_text_" + (args.tag || "").replace(/\./, "_") + " rgraph_accessible_text_" + obj.type + " " + (args.cssClass || "");
                ...
                if (typeof args.angle === "number" && args.angle !== 0) {
                    var coords = RGraph.measureText(args.text, args.bold, args.font, args.size);
                    ...
                    span.style.transformOrigin = "{1} {2}".format(hOrigin, vOrigin);
                    span.style.transform = "rotate(" + args.angle + "deg)";
                }
                // Shadow
                span.style.textShadow = "{1}px {2}px {3}px {4}".format(context.shadowOffsetX, context.shadowOffsetY, context.shadowBlur, context.shadowColor);
                if (args.bounding) {
                ...
                // Pointer events
                if ((typeof obj.properties.textAccessiblePointerevents === "undefined" || obj.properties.textAccessiblePointerevents) && obj.properties.textAccessiblePointerevents !== "none") span.style.pointerEvents = "auto";
                else span.style.pointerEvents = "none";
                span.style.padding = args.bounding ? "2px" : null; // Changed to 2px on 16th January 2019
                span.__text__ = args.text;
                span.insertAdjacentHTML("afterbegin", args.text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace(/\[\[RETURN\]\]/g, "<br />"));
                ...
                // Vertical alignment
                if (args.valign === "top") ;
                else if (args.valign === "center") span.style.top = parseFloat(span.style.top) - span.offsetHeight / 2 + "px";
                else span.style.top = parseFloat(span.style.top) - span.offsetHeight + "px";
                var offsetWidth = parseFloat(span.offsetWidth), offsetHeight = parseFloat(span.offsetHeight), top = parseFloat(span.style.top), left = parseFloat(span.style.left);
                ...
                span.style.display = "inline";
                var offsetWidth = RGraph.text.domNodeDimensionCache[obj.id][cacheKey].width, offsetHeight = RGraph.text.domNodeDimensionCache[obj.id][cacheKey].height, top = RGraph.text.domNodeDimensionCache[obj.id][cacheKey].top, left = RGraph.text.domNodeDimensionCache[obj.id][cacheKey].left;
            }
            ...
            //
            if (obj.type === "drawing.text") // Mousemove
            ...
            // Tooltips
            {
                if (obj.properties.tooltips) span.addEventListener(obj.properties.tooltipsEvent.indexOf("mousemove") !== -1 ? "mousemove" : "click", function(e) {
                    if (!RGraph.Registry.get("tooltip") || RGraph.Registry.get("tooltip").__index__ !== 0 || RGraph.Registry.get("tooltip").__object__.uid != obj.uid) {
                        RGraph.hideTooltip();
                        RGraph.redraw();
            ...
            RGraph.text.domNodeCache.reset = function() {
                ...
                if (arguments[0]) {
                    if (typeof arguments[0] === "string") var canvas = document.getElementById(arguments[0]);
                    else var canvas = arguments[0];
                ...
                // Clear all DOM text from all tags
                    ...
                    for(i1 in RGraph.text.domNodeCache){
                        for(j in RGraph.text.domNodeCache[i1])if (RGraph.text.domNodeCache[i1][j] && RGraph.text.domNodeCache[i1][j].parentNode) RGraph.text.domNodeCache[i1][j].parentNode.removeChild(RGraph.text.domNodeCache[i1][j]);
                    }
            ...
            RGraph.text.find = function(args) {
                var span, nodes = [];
                if (args.object && args.object.isrgraph) var id = args.object.id;
                else if (args.id) {
                ...
                for(i1 in RGraph.text.domNodeCache[id]){
                    span = RGraph.text.domNodeCache[id][i1];
                    // A full tag is given
            ...
            //
            if (obj && obj.isrgraph && obj.coordsText) obj.coordsText.push(ret);
            return ret;
        }
        //
        ...
        if (typeof args.accessible === "undefined") {
            if (obj && obj.properties.textAccessible) return domtext();
        } else if (typeof args.accessible === "boolean" && args.accessible) return domtext();
        var x = args.x, y = args.y, originalX = x, originalY = y, text = args.text, text_multiline = typeof text === "string" ? text.split(/\r?\n/g) : "", numlines = text_multiline.length, font = args.font ? args.font : "Arial", size = args.size ? args.size : 10, size_pixels = size * 1.5, bold = args.bold, italic = args.italic, halign = args.halign ? args.halign : "left", valign = args.valign ? args.valign : "bottom", tag = typeof args.tag == "string" && args.tag.length > 0 ? args.tag : "", marker = args.marker, angle = args.angle || 0;
        //
        // The text arg must be a string or a number
        //
        if (typeof text == "number") text = String(text);
        var bounding = args.bounding, bounding_stroke = args["bounding.stroke"] ? args["bounding.stroke"] : "black", bounding_fill = args["bounding.fill"] ? args["bounding.fill"] : "rgba(255,255,255,0.7)", bounding_shadow = args["bounding.shadow"], bounding_shadow_color = args["bounding.shadow.color"] || "#ccc", bounding_shadow_blur = args["bounding.shadow.blur"] || 3, bounding_shadow_offsetx = args["bounding.shadow.offsetx"] || 3, bounding_shadow_offsety = args["bounding.shadow.offsety"] || 3, bounding_linewidth = typeof args["bounding.linewidth"] === "number" ? args["bounding.linewidth"] : 1;
        //
        ...
        //
        var width = 0;
        for(var i1 = 0; i1 < numlines; ++i1)width = Math.max(width, context.measureText(text_multiline[i1]).width);
        ...
        if (valign == "center") {
            ...
            if (numlines > 1) y -= (numlines - 1) * size_pixels;
        ...
        //
        if (bounding) {
            var pre_bounding_linewidth = context.lineWidth, pre_bounding_strokestyle = context.strokeStyle, pre_bounding_fillstyle = context.fillStyle, pre_bounding_shadowcolor = context.shadowColor, pre_bounding_shadowblur = context.shadowBlur, pre_bounding_shadowoffsetx = context.shadowOffsetX, pre_bounding_shadowoffsety = context.shadowOffsetY;
            ...
            context.fillRect(boundingX, boundingY, boundingW, boundingH);
            context.strokeRect(boundingX, boundingY, boundingW, boundingH);
        ...
        // Draw the text
        //
        if (numlines > 1) for(var i1 = 0; i1 < numlines; ++i1)context.fillText(text_multiline[i1], x, y + size_pixels * i1);
        else context.fillText(text, x + 0.5, y + 0.5);
        //
        ...
        if (angle != 0) {
            if (angle == 90) {
                if (halign == "left") {
                    ...
                    if (valign == "center") {
                        boundingX = originalX - height / 2 - 2;
                        boundingY = originalY - 2;
                        boundingW = height + 4;
                        boundingH = width + 4;
                    }
                    if (valign == "top") {
                        boundingX = originalX - height - 2;
                        boundingY = originalY - 2;
                        boundingW = height + 4;
                        boundingH = width + 4;
                    }
                ...
                if (halign == "left") {
                    ...
                    if (valign == "center") {
                        boundingX = originalX - width - 2;
                        boundingY = originalY - height / 2 - 2;
                        boundingW = width + 4;
                        boundingH = height + 4;
                    }
                    if (valign == "top") {
                        boundingX = originalX - width - 2;
                        boundingY = originalY - height - 2;
                        boundingW = width + 4;
                        boundingH = height + 4;
                    }
                } else if (halign == "center") {
                    if (valign == "bottom") {
                        boundingX = originalX - width / 2 - 2;
                        boundingY = originalY - 2;
                        boundingW = width + 4;
                        boundingH = height + 4;
                    }
                ...
                if (halign == "left") {
                    ...
                    if (valign == "center") {
                        boundingX = originalX - height / 2 - 4;
                        boundingY = originalY - width - 2;
                        boundingW = height + 4;
                        boundingH = width + 4;
                    }
    ...
    RGraph.addCustomText = function(obj) {
        if (RGraph.isArray(obj.properties.text) && obj.properties.text.length) for(var i1 = 0; i1 < obj.properties.text.length; ++i1){
            var conf = obj.properties.text[i1];
            // Add the object to the config
            conf.object = obj;
            // Set the color to black if it's not set
            if (typeof conf.color !== "string" || !conf.color.length) conf.color = "black";
            RGraph.text(conf);
        }
    };
    //
    ...
    RGraph.groupedIndexToSequential = function() {
        ...
        for(var i1 = 0, seq = 0; i1 <= args.dataset; ++i1)for(var j1 = 0; j1 < args.object.data[args.dataset].length; ++j1){
            if (i1 === args.dataset && j1 === args.index) return seq;
            seq++;
        }
        return seq;
    };
    ...
    RGraph.Highlight.rect = function() {
        var args = RGraph.getArgs(arguments, "object,shape"), properties = args.object.properties;
        if (properties.tooltipsHighlight) {
            ...
            args.object.context.rect(args.shape.x - 0.5, args.shape.y - 0.5, args.shape.width + 1, args.shape.height + 1);
    ...
    RGraph.Highlight.point = function() {
        ...
        if (properties.tooltipsHighlight) {
            ...
            args.object.context.arc(args.shape.x, args.shape.y, radius, 0, RGraph.TWOPI, 0);
            args.object.context.stroke();
            args.object.context.fill();
        }
    };
    ...
    RGraph.parseDate = function() {
        ...
        if (args.str.match(/^\d\d\d\d-\d\d-\d\d(t|T)\d\d:\d\d(:\d\d)?$/)) args.str = args.str.toUpperCase().replace(/T/, " ");
        var d = new Date();
        ...
        for(var i1 = 0; i1 < months.length; ++i1){
            months[months[i1]] = i1;
            months[months[i1].substring(0, 3)] = i1;
            months_regex = months_regex + "|" + months[i1].substring(0, 3);
        }
        ...
        for(var i1 = 0, len = tokens.length; i1 < len; ++i1)if (tokens[i1]) {
            // Year
            if (tokens[i1].match(/^\d\d\d\d$/)) defaults.year = tokens[i1];
            // Month
            var res = isMonth(tokens[i1]);
            if (typeof res === "number") defaults.month = res + 1; // Months are zero indexed
            // Date
            if (tokens[i1].match(/^\d?\d(?:st|nd|rd|th)?$/)) defaults.date = parseInt(tokens[i1]);
            // Time
            if (tokens[i1].match(/^(\d\d):(\d\d):?(?:(\d\d))?$/)) {
                defaults.hours = parseInt(RegExp.$1);
                defaults.minutes = parseInt(RegExp.$2);
                if (RegExp.$3) defaults.seconds = parseInt(RegExp.$3);
            }
            // Dateformat: XXXX-XX-XX
            if (tokens[i1].match(new RegExp("^(\\d\\d\\d\\d)" + sep + "(\\d\\d)" + sep + "(\\d\\d)$", "i"))) {
                defaults.date = parseInt(RegExp.$3);
                defaults.month = parseInt(RegExp.$2);
                defaults.year = parseInt(RegExp.$1);
            }
            // Dateformat: XX-XX-XXXX
            if (tokens[i1].match(new RegExp("^(\\d\\d)" + sep + "(\\d\\d)" + sep + "(\\d\\d\\d\\d)$", "i"))) {
        ...
        args.str = "{1}/{2}/{3} {4}:{5}:{6}".format(defaults.year, String(defaults.month).length === 1 ? "0" + defaults.month : defaults.month, String(defaults.date).length === 1 ? "0" + defaults.date : defaults.date, String(defaults.hours).length === 1 ? "0" + defaults.hours : defaults.hours, String(defaults.minutes).length === 1 ? "0" + defaults.minutes : defaults.minutes, String(defaults.seconds).length === 1 ? "0" + defaults.seconds : defaults.seconds);
    ...
    RGraph.parseDateOld = function() {
        ...
        if (args.str === "now") args.str = new Date().toString();
    ...
    RGraph.resetColorsToOriginalValues = function() {
        ...
        if (args.object.original_colors) {
            ...
            for(var j1 in args.object.original_colors)if (typeof j1 === "string") args.object.properties[j1] = RGraph.arrayClone(args.object.original_colors[j1]);
        }
        //
    ...
    RGraph.linearGradient = function() {
        ...
        for(var i1 = 0; i1 < args.colors.length; ++i1){
            var color = args.colors[i1];
            var stop = i1 / (numColors - 1);
    ...
    RGraph.radialGradient = function() {
        var args = arguments[0];
        var gradient = args.object.context.createRadialGradient(args.x1, args.y1, args.r1, args.x2, args.y2, args.r2);
        var numColors = args.colors.length;
        for(var i1 = 0; i1 < args.colors.length; ++i1)gradient.addColorStop(i1 / (numColors - 1), args.colors[i1]);
        return gradient;
    };
    //
    ...
    RGraph.arrayRandom = function() {
        ...
        for(var i1 = 0, arr = []; i1 < args.count; i1 += 1)arr.push(RGraph.random(args.min, args.max, args.decimals));
        return arr;
    };
    //
    ...
    RGraph.setShadow = function() {
        // 1 Argument
        if (typeof arguments[0] === "object" && typeof arguments[0].object === "object" && typeof arguments[0].object.isrgraph && typeof arguments[0].prefix === "string") {
            var args = arguments[0];
        ...
        // Turn Off the shadow
        } else if (arguments.length === 1 && typeof arguments[0] === "object" && typeof arguments[0].isrgraph) {
            var obj = arguments[0];
    ...
    RGraph.isArray = function() {
        var args = RGraph.getArgs(arguments, "object");
        if (args.object && args.object.constructor) var pos = args.object.constructor.toString().indexOf("Array");
        else return false;
        return args.object != null && typeof pos === "number" && pos > 0 && pos < 20;
    };
    //
    ...
    RGraph.isNull = function() {
        ...
        if (typeof args.arg === "object" && !args.arg) return true;
        return false;
    };
    //
    ...
    RGraph.reset = function() {
        ...
        if (typeof args.canvas === "string") args.canvas = document.getElementById(args.canvas);
    ...
    RGraph.Effects.updateCanvas = function() {
        ...
        window.requestAnimationFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame || window.mozRequestAnimationFrame || function(func) {
    ...
    RGraph.stringsToNumbers = function() {
        ...
        if (typeof args.string === "string" && args.string.trim().match(/^\[ *\d+$/)) args.string = args.string.replace("[", "");
        // If it's already a number just return it
        if (typeof args.string === "number") return args.string;
        if (typeof args.string === "string") {
            if (args.string.indexOf(sep) != -1) args.string = args.string.split(sep);
            else {
                args.string = parseFloat(args.string);
                if (isNaN(args.string)) args.string = null;
            }
        }
        if (typeof args.string === "object" && !RGraph.isNull(args.string)) for(var i1 = 0, len = args.string.length; i1 < len; i1 += 1)args.string[i1] = RGraph.stringsToNumbers(args.string[i1], args.separator);
        return args.string;
    };
    //
    ...
    RGraph.cachedDraw = function() {
        ...
        if (!RGraph.cache[args.id]) {
            ...
            // Call the function
            args.function(args.object, RGraph.cache[args.id].canvas, RGraph.cache[args.id].context);
        }
    ...
    RGraph.parseObjectStyleConfig = function() {
        ...
        for(var i1 in args.config)if (typeof i1 === "string") args.object.set(i1, args.config[i1]);
    };
    //
    ...
    RGraph.path = function(args) {
        ...
        for(var i1 = 0, len = p.length; i1 < len; i1 += 1)switch(p[i1]){
            case "b":
                context.beginPath();
                break;
            case "c":
                ...
                break;
            case "m":
                context.moveTo(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]));
                i1 += 2;
                break;
            case "l":
                context.lineTo(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]));
                i1 += 2;
                break;
            case "s":
                if (p[i1 + 1]) context.strokeStyle = p[i1 + 1];
                context.stroke();
                i1++;
                break;
            case "f":
                if (p[i1 + 1]) context.fillStyle = p[i1 + 1];
                context.fill();
                i1++;
                break;
            case "qc":
                context.quadraticCurveTo(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]));
                i1 += 4;
                break;
            case "bc":
                context.bezierCurveTo(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]), parseFloat(p[i1 + 5]), parseFloat(p[i1 + 6]));
                i1 += 6;
                break;
            case "r":
                context.rect(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]));
                i1 += 4;
                break;
            case "a":
                context.arc(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]), parseFloat(p[i1 + 5]), p[i1 + 6] === "true" || p[i1 + 6] === true || p[i1 + 6] === 1 || p[i1 + 6] === "1" ? true : false);
                i1 += 6;
                break;
            case "at":
                context.arcTo(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]), parseFloat(p[i1 + 5]));
                i1 += 5;
                break;
            case "lw":
                context.lineWidth = parseFloat(p[i1 + 1]);
                i1++;
                break;
            case "e":
                context.ellipse(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]), parseFloat(p[i1 + 5]), parseFloat(p[i1 + 6]), parseFloat(p[i1 + 7]), p[i1 + 8] === "true" ? true : false);
                i1 += 8;
                break;
            case "lj":
                context.lineJoin = p[i1 + 1];
                i1++;
                break;
            case "lc":
                context.lineCap = p[i1 + 1];
                i1++;
                break;
            case "sc":
                context.shadowColor = p[i1 + 1];
                i1++;
                break;
            case "sb":
                context.shadowBlur = parseFloat(p[i1 + 1]);
                i1++;
                break;
            case "sx":
                context.shadowOffsetX = parseFloat(p[i1 + 1]);
                i1++;
                break;
            case "sy":
                context.shadowOffsetY = parseFloat(p[i1 + 1]);
                i1++;
                break;
            case "fs":
                context.fillStyle = p[i1 + 1];
                i1++;
                break;
            case "ss":
                context.strokeStyle = p[i1 + 1];
                i1++;
                break;
            case "fr":
                context.fillRect(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]));
                i1 += 4;
                break;
            case "sr":
                context.strokeRect(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]));
                i1 += 4;
                break;
            case "cl":
                context.clip();
            ...
            case "rs":
                ...
                break;
            case "tr":
                context.translate(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]));
                i1 += 2;
                break;
            case "sl":
                context.scale(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]));
                i1 += 2;
                break;
            case "ro":
                context.rotate(parseFloat(p[i1 + 1]));
                i1++;
                break;
            case "tf":
                context.transform(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]), parseFloat(p[i1 + 5]), parseFloat(p[i1 + 6]));
                i1 += 6;
                break;
            case "stf":
                context.setTransform(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]), parseFloat(p[i1 + 5]), parseFloat(p[i1 + 6]));
                i1 += 6;
                break;
            case "cr":
                context.clearRect(parseFloat(p[i1 + 1]), parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]), parseFloat(p[i1 + 4]));
                i1 += 4;
                break;
            case "ld":
                var parts = p[i1 + 1];
            ...
            case "fo":
                ...
                break;
            case "ft":
                context.fillText(p[i1 + 1], parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]));
                i1 += 3;
                break;
            case "st":
                context.strokeText(p[i1 + 1], parseFloat(p[i1 + 2]), parseFloat(p[i1 + 3]));
                i1 += 3;
                break;
            case "ta":
                context.textAlign = p[i1 + 1];
                i1++;
                break;
            case "tbl":
                context.textBaseline = p[i1 + 1];
                i1++;
                break;
            case "ga":
                context.globalAlpha = parseFloat(p[i1 + 1]);
                i1++;
                break;
            case "gco":
                context.globalCompositeOperation = p[i1 + 1];
                i1++;
                break;
            case "fu":
                p[i1 + 1](context.canvas.__object__);
                i1++;
                break;
            ...
            // Unknown option
            default:
                alert("[ERROR] Unknown option: " + p[i1]);
        }
        function splitstring(p) {
            var ret = [], buffer = "", inquote = false, quote = "", substitutionIndex = 0;
            // p is a string - not an array
            for(var i1 = 0; i1 < p.length; i1 += 1){
                var chr = p[i1], isWS = chr.match(/ /);
                if (isWS) {
                    if (!inquote) {
                        // Get rid of any enclosing quotes
                        if (buffer[0] === '"' || buffer[0] === "'") buffer = buffer.substr(1, buffer.length - 2);
                        // String substitution
    ...
    RGraph.getTextConf = function(args) {
        var obj = args.object, properties = obj.properties, prefix1 = args.prefix;
        // Has to be a seperate var statement
        var font = typeof properties[prefix1 + "Font"] === "string" ? properties[prefix1 + "Font"] : properties.textFont, size = typeof properties[prefix1 + "Size"] === "number" ? properties[prefix1 + "Size"] : properties.textSize, color = typeof properties[prefix1 + "Color"] === "string" ? properties[prefix1 + "Color"] : properties.textColor, bold = !RGraph.isNull(properties[prefix1 + "Bold"]) ? properties[prefix1 + "Bold"] : properties.textBold, italic = !RGraph.isNull(properties[prefix1 + "Italic"]) ? properties[prefix1 + "Italic"] : properties.textItalic;
        return {
            font: font,
            size: size,
            color: color,
    ...
    RGraph.responsive = function(conf) {
        ...
        for(var i1 = 0; i1 < conf.length; ++i1){
            if (conf[i1 + 1] && typeof conf[i1 + 1].maxWidth === "number") conf[i1].minWidth = conf[i1 + 1].maxWidth;
            else if (!conf[i1 + 1]) conf[i1].minWidth = 0;
        }
        //
        ...
        for(var i1 = 0; i1 < conf.length; ++i1){
            ...
            conf[i1].minWidth = RGraph.isNull(conf[i1].minWidth) ? 0 : conf[i1].minWidth;
            conf[i1].maxWidth = RGraph.isNull(conf[i1].maxWidth) ? 100000 : conf[i1].maxWidth;
            // Create the media query string
            var str = "screen and (min-width: %1px) and (max-width: %2px)".format(conf[i1].minWidth, conf[i1].maxWidth);
            var mediaQuery = window.matchMedia(str);
            (function(index) {
                mediaQuery.addListener(function(e) {
                    if (e.matches) matchFunction(conf[index]);
                });
            })(i1);
            // An Initial test
            if (document.documentElement.clientWidth >= conf[i1].minWidth && document.documentElement.clientWidth < conf[i1].maxWidth) matchFunction(conf[i1]);
        }
        //
        ...
        function matchFunction(rule) {
            ...
            if (typeof rule.width === "number") {
                if (obj.get("textAccessible")) obj.canvas.parentNode.style.width = rule.width + "px";
            ...
            if (typeof rule.height === "number") {
                if (obj.get("textAccessible")) obj.canvas.parentNode.style.height = rule.height + "px";
            ...
            if (typeof rule.options === "object") {
                for(var j1 in rule.options)if (typeof j1 === "string") obj.set(j1, rule.options[j1]);
            }
            //
            ...
            var setCSS = function(el, name, value) {
                ...
                for(var i1 = 0; i1 < replacements.length; ++i1)if (name === replacements[i1][0]) name = replacements[i1][1];
                el.style[name] = value;
            };
            //
            ...
            if (typeof rule.css === "object") {
                for(var j1 in rule.css)if (typeof j1 === "string") {
                    if (obj.get("textAccessible")) setCSS(obj.canvas.parentNode, j1, rule.css[j1]);
                    else setCSS(obj.canvas, j1, rule.css[j1]);
                }
            }
            //
            ...
            if (typeof rule.parentCss === "object") {
                for(var j1 in rule.parentCss)if (typeof j1 === "string") {
                    if (obj.get("textAccessible")) setCSS(obj.canvas.parentNode.parentNode, j1, rule.parentCss[j1]);
                    else setCSS(obj.canvas.parentNode, j1, rule.parentCss[j1]);
                }
            }
            RGraph.cache = [];
            RGraph.resetColorsToOriginalValues(obj);
            if (obj.get("textAccessible") && RGraph.text.domNodeCache && RGraph.text.domNodeCache.reset) RGraph.text.domNodeCache.reset(obj.canvas);
            RGraph.redraw(obj.canvas);
            // Run the callback function if it's defined
            if (typeof rule.callback === "function") rule.callback(obj);
        }
    ...
    RGraph.responsive_old = function(conf) {
        var args = arguments[1] || {}, // This function is added to each object in their constructors so the this
        // variable is the chart object.
        obj = this, // The func variable becomes the function that is fired by the resize event
        func = null, // This is the timer reference
        timer = null;
        ...
        var func = function() {
            ...
            //
            if (!matched && (document.documentElement.clientWidth <= conf[i1].maxWidth || RGraph.isNull(conf[i1].maxWidth))) {
                matched = true;
                // If a width is defined for this rule set it
                if (typeof conf[i1].width === "number") {
                    if (obj.get("textAccessible")) obj.canvas.parentNode.style.width = conf[i1].width + "px";
                    obj.canvas.width = conf[i1].width;
                    obj.canvas.__rgraph_aa_translated__ = false;
                }
                //
                // If a height is defined for this rule set it
                //
                if (typeof conf[i1].height === "number") {
                    if (obj.get("textAccessible")) obj.canvas.parentNode.style.height = conf[i1].height + "px";
                    obj.canvas.height = conf[i1].height;
                    obj.canvas.__rgraph_aa_translated__ = false;
                }
                //
                // Are there any options to be set?
                //
                if (typeof conf[i1].options === "object" && typeof conf[i1].options === "object") {
                    for(var j1 in conf[i1].options)if (typeof j1 === "string") obj.set(j1, conf[i1].options[j1]);
                }
                //
                // This function simply sets a CSS property on the object.
                // It accommodates certain name changes
                ...
                // Are there any CSS properties to set on the canvas tag?
                //
                if (typeof conf[i1].css === "object") {
                    for(var j1 in conf[i1].css)if (typeof j1 === "string") {
                        if (obj.get("textAccessible")) setCSS(obj.canvas.parentNode, j1, conf[i1].css[j1]);
                        else setCSS(obj.canvas, j1, conf[i1].css[j1]);
                    }
                }
                //
                // Are there any CSS properties to set on the canvas tahs PARENT?
                //
                if (typeof conf[i1].parentCss === "object") {
                    for(var j1 in conf[i1].parentCss)if (typeof j1 === "string") {
                        if (obj.get("textAccessible")) setCSS(obj.canvas.parentNode.parentNode, j1, conf[i1].parentCss[j1]);
                        else setCSS(obj.canvas.parentNode, j1, conf[i1].parentCss[j1]);
                    }
                }
                // Redraw the chart
                RGraph.cache = [];
                RGraph.resetColorsToOriginalValues(obj);
                if (obj.get("textAccessible")) RGraph.text.domNodeCache.reset(obj.canvas);
                RGraph.redraw();
                // Run the callback function if it's defined
                if (typeof conf[i1].callback === "function") conf[i1].callback(obj);
            }
        };
    ...
    RGraph.drawXAxis = function(obj) {
        var properties = obj.properties, context = obj.context, tickmarksLength1 = typeof properties.xaxisTickmarksLength === "number" ? properties.xaxisTickmarksLength : 3, isSketch = obj.type === "bar" && properties.variant === "sketch";
        //
        // If the xaxisLabels property is defined then go through it converting
        // null and undefined values to empty strings.
        //
        if (typeof properties.xaxisLabels === "object" && !RGraph.isNull(properties.xaxisLabels) && properties.xaxisLabels.length) {
            for(var i1 = 0; i1 < properties.xaxisLabels.length; ++i1)if (typeof properties.xaxisLabels[i1] === "undefined" || properties.xaxisLabels[i1] === null) properties.xaxisLabels[i1] = "";
        }
        //
        // Calculate the Y coordinate for the X axis
        if ((obj.type === "hbar" || obj.type === "gantt") && properties.xaxisPosition === "bottom") var y = obj.canvas.height - properties.marginBottom;
        else if ((obj.type === "hbar" || obj.type === "gantt") && properties.xaxisPosition === "top") var y = properties.marginTop;
        else var y = obj.getYCoord(properties.yaxisScaleMin > 0 ? properties.yaxisScaleMin : 0);
        ...
        if (obj.type === "scatter" && properties.yaxisScaleInvert) {
            if (properties.yaxisScaleMin >= 0) y = obj.getYCoord(obj.scale2.max);
        }
        // Special case for positioning an X axis Drawing API object
        if (obj.type === "drawing.xaxis") {
            if (properties.xaxisPosition === "center") y = (obj.canvas.height - properties.marginTop - properties.marginBottom) / 2 + properties.marginTop;
            else y = obj.y;
        }
        //
        // Draw the axis
        ...
        if (properties.xaxis) {
            // Draw the axis
            obj.path("lw % b m % % l % % s %", properties.xaxisLinewidth, properties.marginLeft - (isSketch ? 5 : 0), y - (isSketch ? 2 : 0), obj.canvas.width - properties.marginRight + (isSketch ? 7 : 0), y + (isSketch ? 2 : 0), properties.xaxisColor);
            // Draw the tickmarks if necessary
            if (!isSketch) {
                if (properties.xaxisTickmarks) {
                    if (typeof properties.xaxisTickmarksCount === "number") var xaxisTickmarksCount = properties.xaxisTickmarksCount;
                    else if (obj.type === "bar") var xaxisTickmarksCount = obj.data.length || 10;
                    else if (obj.type === "hbar") var xaxisTickmarksCount = properties.xaxisLabelsCount || 5;
                    else if (obj.type === "line") var xaxisTickmarksCount = obj.data[0].length > 0 ? obj.data[0].length - 1 : 10;
                    else if (obj.type === "scatter" && properties.scale) var xaxisTickmarksCount = 5;
                    else if (obj.type === "scatter" && properties.xaxisLabels) var xaxisTickmarksCount = properties.xaxisLabels.length;
                    else if (obj.type === "scatter") var xaxisTickmarksCount = 5;
                    else if (obj.type === "waterfall") var xaxisTickmarksCount = obj.data.length + (properties.total ? 1 : 0);
                    else if (obj.type === "drawing.xaxis") {
                        if (properties.scale) var xaxisTickmarksCount = properties.xaxisScaleLabelsCount;
                        else if (properties.xaxisLabels && properties.xaxisLabels.length) var xaxisTickmarksCount = properties.xaxisLabels.length;
                        else var xaxisTickmarksCount = 5;
                    //  Default to 5 tickmarks
                    } else xaxisTickmarksCount = 5;
                } else properties.xaxisTickmarksCount = 0;
                ...
                if (properties.xaxisPosition === "center" && properties.yaxisScaleMin >= 0) {
                    if (properties.yaxisScaleInvert) {
                        var tickmarksYStart = obj.getYCoord(properties.yaxisScaleMax) - tickmarksLength1;
                        var tickmarksYEnd = obj.getYCoord(properties.yaxisScaleMax) + tickmarksLength1;
                    } else {
                        var tickmarksYStart = obj.getYCoord(properties.yaxisScaleMin) - tickmarksLength1;
                        var tickmarksYEnd = obj.getYCoord(properties.yaxisScaleMin) + tickmarksLength1;
                    }
                } else if (properties.xaxisPosition === "center" || properties.yaxisScaleMin < 0) {
                    if (properties.yaxisScaleInvert) {
                        var tickmarksYStart = obj.getYCoord(obj.scale2.max) - tickmarksLength1;
                        var tickmarksYEnd = obj.getYCoord(obj.scale2.max) + tickmarksLength1;
                    } else {
                        var tickmarksYStart = obj.getYCoord(0) - tickmarksLength1;
                        var tickmarksYEnd = obj.getYCoord(0) + tickmarksLength1;
                    }
                    ...
                    if (obj.getYCoord) {
                        ...
                        var tickmarksYStart = properties.marginTop - tickmarksLength1;
                        var tickmarksYEnd = properties.marginTop;
                    }
                } else if (obj.getYCoord) {
                    if (obj.type === "line" && properties.yaxisScaleInvert) {
                        ...
                        tickmarksYEnd = obj.y + tickmarksLength1;
                    } else {
                    ...
                    var tickmarksYEnd = obj.canvas.height - properties.marginBottom + tickmarksLength1;
                }
                //if (!xaxisTickmarksCount) {
                //xaxisTickmarksCount = 10;
                //}
                for(var i1 = 0; i1 <= xaxisTickmarksCount; ++i1){
                    ...
                    if (RGraph.isNull(properties.xaxisTickmarksLastLeft)) {
                        if (i1 === 0 && properties.yaxis && properties.yaxisPosition === "left" && !(properties.xaxisScaleMax > 0 && properties.xaxisScaleMin < 0)) continue;
                    } else if (i1 === 0 && !properties.xaxisTickmarksLastLeft) continue;
                    ...
                    if (RGraph.isNull(properties.xaxisTickmarksLastRight)) {
                        if (i1 === xaxisTickmarksCount && properties.yaxis && properties.yaxisPosition === "right") continue;
                    } else if (i1 === xaxisTickmarksCount && !properties.xaxisTickmarksLastRight) continue;
                    var x = (obj.canvas.width - properties.marginLeft - properties.marginRight) / xaxisTickmarksCount * i1 + properties.marginLeft;
                    // If the chart is an HBar and the Y axis is in the center
                    // then don't draw a tickmark at the same position as the
                    // Y axis.
                    if (obj.type === "hbar") {
                        if (properties.yaxisPosition === "center" && x > obj.getXCoord(0) - 2 && x < obj.getXCoord(0) + 2) continue;
                        else if (properties.yaxisPosition === "left" && properties.xaxisScaleMin < 0 && properties.xaxisScaleMax > 0 && x > obj.getXCoord(0) - 2 && x < obj.getXCoord(0) + 2) continue;
                    }
        ...
        if (properties.xaxisLabelsAngle != 0) {
            ...
            if (properties.xaxisPosition === "top") var angle = properties.xaxisLabelsAngle;
        } else {
            var valign = "top";
            var halign = "center";
            var angle = 0;
        ...
        if (properties.xaxisScale) {
            ...
            //
            if (obj.type === "scatter") scale = obj.xscale2;
            else if (obj.type === "drawing.xaxis") {
                if (properties.xaxisScale) {
                    scale = RGraph.getScale({
                        object: this,
                        options: {
                    ...
                    for(var i1 = 0; i1 <= properties.xaxisScaleLabelsCount; ++i1){
                        var original = (properties.xaxisScaleMax - properties.xaxisScaleMin) / properties.xaxisScaleLabelsCount * i1 + properties.xaxisScaleMin;
                        var hmargin = properties.marginInner;
                        if (typeof properties.xaxisScaleFormatter === "function") var text = properties.xaxisScaleFormatter(this, original);
                        else text = RGraph.numberFormat({
                            object: obj,
            ...
            for(var i1 = 0; i1 < scale.labels.length; ++i1){
                var section = (obj.canvas.width - properties.marginLeft - properties.marginRight) / scale.labels.length;
                if (properties.yaxisPosition === "right") var x = properties.marginLeft + section * i1;
                else if (properties.yaxisPosition === "center") {
                    section /= 2;
                    var x = obj.getXCoord(0) + section + section * i1;
                } else var x = properties.marginLeft + section * i1 + section;
                ...
                if (obj.type === "drawing.xaxis") {
                    if (properties.xaxisPosition === "top") y = obj.y - (properties.xaxisTickmarksLength || 0) + properties.xaxisLabelsOffsety;
                    else y = obj.y + (properties.xaxisTickmarksLength || 0) + properties.xaxisLabelsOffsety + 5;
                }
                RGraph.text({
                    object: obj,
                    ...
                    y: y,
                    text: typeof properties.xaxisScaleFormatter === "function" ? properties.xaxisScaleFormatter({
                        object: obj,
                        number: scale.values[i1]
                    }) : properties.yaxisPosition === "right" ? String(scale.labels[scale.labels.length - 1 - i1]) : String(scale.labels[i1]),
                ...
                if (obj.type === "hbar" && properties.yaxisPosition === "center") {
                    x = obj.getXCoord(0) - section - section * i1;
                    RGraph.text({
                        object: obj,
                        textConfPrefix: "xaxisLabels",
                        x: x + properties.xaxisLabelsOffsetx,
            ...
            if (properties.xaxisLabelsPosition === "edge") var section = (obj.canvas.width - properties.marginLeft - properties.marginRight - (properties.marginInner || 0) - (properties.marginInner || 0)) / (properties.xaxisLabels.length - 1);
            else var section = (obj.canvas.width - properties.marginLeft - properties.marginRight) / properties.xaxisLabels.length;
            for(var i1 = 0; i1 < properties.xaxisLabels.length; ++i1){
                if (properties.xaxisLabelsPosition === "edge") var x = properties.marginLeft + (properties.marginInner || 0) + section * i1;
                else var x = properties.marginLeft + section * i1 + section / 2;
                // Allow for the Scatter chart labels to be at specific points
                // along the X scale
                if (typeof properties.xaxisLabels[i1] === "object" && obj.type === "scatter") {
                    var rightEdge = 0;
                    var align = properties.xaxisLabelsSpecificAlign === "left" ? "left" : "center";
                    var halign = "center";
                    if (properties.xaxisLabels[i1 + 1] && typeof properties.xaxisLabels[i1 + 1][1] === "number") rightEdge = properties.xaxisLabels[i1 + 1][1];
                    else rightEdge = properties.xaxisScaleMax;
                    var leftEdge = properties.xaxisLabels[i1][1];
                    ...
                    var ret = RGraph.text({
                        ...
                        text: String(properties.xaxisLabels[i1][0]),
                        angle: angle,
                        marker: false,
                        tag: "labels.specific",
                    ...
                    //
                    obj.path("b m % % l % % s #bbb", Math.round(properties.marginLeft + (properties.xaxisLabels[i1][1] - properties.xaxisScaleMin) / (properties.xaxisScaleMax - properties.xaxisScaleMin) * (obj.canvas.width - properties.marginLeft - properties.marginRight)), obj.canvas.height - properties.marginBottom, Math.round(properties.marginLeft + (properties.xaxisLabels[i1][1] - properties.xaxisScaleMin) / (properties.xaxisScaleMax - properties.xaxisScaleMin) * (obj.canvas.width - properties.marginLeft - properties.marginRight)), obj.canvas.height - properties.marginBottom + 20);
                    // Draw the final indicator line if we're on the final label
                    if (i1 === properties.xaxisLabels.length - 1) obj.path("b m % % l % % s #bbb", obj.canvas.width - properties.marginRight, obj.canvas.height - properties.marginBottom, obj.canvas.width - properties.marginRight, obj.canvas.height - properties.marginBottom + 20);
                // A regular label
                } else {
                    ...
                    if (obj.type === "drawing.xaxis") //y = obj.y + (properties.xaxisTickmarksLength || 0) + properties.xaxisLabelsOffsety + 5;
                    {
                        if (obj.type === "drawing.xaxis") {
                            if (properties.xaxisPosition === "top") y = obj.y - (properties.xaxisTickmarksLength || 0) + properties.xaxisLabelsOffsety;
                            else y = obj.y + (properties.xaxisTickmarksLength || 0) + properties.xaxisLabelsOffsety + 5;
                    ...
                    var ret = RGraph.text({
                        ...
                        text: String(properties.xaxisLabels[i1]),
                        valign: typeof properties.xaxisLabelsValign === "string" ? properties.xaxisLabelsValign : properties.xaxisPosition === "top" ? "bottom" : valign,
                        halign: typeof properties.xaxisLabelsHalign === "string" ? properties.xaxisLabelsHalign : halign,
                        marker: false,
                        angle: angle,
        ...
        if (properties.xaxisTitle) {
            ...
            if (obj.type === "drawing.xaxis") y = obj.y + 7 + properties.xaxisTitleOffsety + properties.xaxisTickmarksLength;
            ...
            if (properties.xaxisScale || properties.xaxisLabels && properties.xaxisLabels.length) {
                ...
                if (properties.xaxisPosition === "top") y -= textConf.size * 1.5;
                else y += textConf.size * 1.5;
            }
            // The xaxisTitlePos property
            if (typeof properties.xaxisTitlePos === "number") {
                if (properties.xaxisPosition === "top") y = properties.marginTop * properties.xaxisTitlePos;
                else y = obj.canvas.height - properties.marginBottom + properties.marginBottom * properties.xaxisTitlePos;
            }
    ...
    RGraph.drawYAxis = function(obj) {
        var properties = obj.properties, context = obj.context, isSketch = obj.type === "bar" && properties.variant === "sketch";
        ...
        if (obj.type === "hbar") properties.yaxisLabelsSpecific = properties.yaxisLabels;
        ...
        if ((obj.type === "hbar" || obj.type === "gantt") && properties.yaxisPosition === "left") {
            ...
            if (obj.type === "hbar" && properties.xaxisScaleMin > 0 && properties.xaxisScaleMax > properties.xaxisScaleMin) var x = obj.getXCoord(properties.xaxisScaleMin);
        } else if ((obj.type === "hbar" || obj.type === "gantt") && properties.yaxisPosition === "right") var x = obj.canvas.width - properties.marginRight;
        else if (obj.type === "drawing.yaxis") var x = obj.x;
        else var x = properties.yaxisPosition === "right" ? obj.canvas.width - properties.marginRight : obj.type === "hbar" ? obj.getXCoord(0) : properties.marginLeft;
        //
        // Draw the Y axis
        //
        if (properties.yaxis) {
            // Draw the axis
            obj.path("lw % b m % % l % % s %", properties.yaxisLinewidth, x - (isSketch ? 5 : 0), properties.marginTop - (isSketch ? 2 : 0), x + (isSketch ? 7 : 0), obj.canvas.height - properties.marginBottom + (isSketch ? 2 : 0), properties.yaxisColor);
            // Draw the tickmarks for the Y axis if necessary
            if (!isSketch) {
                if (properties.yaxisTickmarks) {
                    if (typeof properties.yaxisTickmarksCount === "number") var yaxisTickmarksCount = properties.yaxisTickmarksCount;
                    else if (obj.type === "bar") var yaxisTickmarksCount = properties.yaxisLabelsSpecific ? properties.yaxisLabelsSpecific.length - 1 : properties.yaxisLabelsCount;
                    else if (obj.type === "hbar") var yaxisTickmarksCount = obj.data.length || 5;
                    else if (obj.type === "line") var yaxisTickmarksCount = properties.yaxisLabelsSpecific ? properties.yaxisLabelsSpecific.length - 1 : properties.yaxisLabelsCount;
                    else if (obj.type === "scatter") var yaxisTickmarksCount = properties.yaxisLabelsCount;
                    else if (obj.type === "waterfall") var yaxisTickmarksCount = properties.yaxisLabelsCount;
                    else yaxisTickmarksCount = 5;
                } else properties.yaxisTickmarksCount = 0;
                ...
                //
                for(var i1 = 0; i1 <= yaxisTickmarksCount; ++i1){
                    ...
                    if (RGraph.isNull(properties.yaxisTickmarksLastTop)) {
                        if (i1 === 0 && properties.xaxis && properties.xaxisPosition === "top") continue;
                    } else if (i1 === 0 && !properties.yaxisTickmarksLastTop) continue;
                    ...
                    if (RGraph.isNull(properties.yaxisTickmarksLastBottom)) {
                        if (i1 === yaxisTickmarksCount && properties.xaxis && properties.xaxisPosition === "bottom") continue;
                    } else if (i1 === yaxisTickmarksCount && !properties.yaxisTickmarksLastBottom) continue;
                    var y = (obj.canvas.height - properties.marginTop - properties.marginBottom) / yaxisTickmarksCount * i1 + properties.marginTop;
                    if (obj.getYCoord) var xaxisYCoord = obj.getYCoord(0);
                    else if (obj.getXCoord) var xaxisYCoord = obj.getXCoord(0);
                    else var xaxisYCoord = obj.marginTop;
                    ...
                    obj.path("b m % % l % % s %", tickmarksXStart, y, tickmarksXEnd, y, properties.yaxisColor);
                    // if the X axis is offset (eg -10,0,10,20,30,40) draw an extra
                    // tickmark at the bottom of the axes
                    if (properties.yaxisScaleMin < 0 && properties.yaxisScaleMax > 0) obj.path("b m % % l % % s %", tickmarksXStart, obj.canvas.height - properties.marginBottom, tickmarksXEnd, obj.canvas.height - properties.marginBottom, properties.yaxisColor);
                    // if the chart is:
                    // o A Scatter chart
        ...
        //
        var valign = "center", halign = "right", angle = 0;
        //
        // Draw a Y axis scale.
        //
        if (properties.yaxisScale && !properties.yaxisLabelsSpecific) {
            if (obj.type === "drawing.yaxis") {
                if (properties.yaxisScale) {
                    ...
                    for(var i1 = 0; i1 <= properties.yaxisScaleLabelsCount; ++i1){
                        var original = (properties.yaxisScaleMax - properties.yaxisScaleMin) / properties.yaxisScaleLabelsCount * i1 + properties.yaxisScaleMin;
                        var hmargin = properties.marginInner;
                        if (typeof properties.yaxisScaleFormatter === "function") var text = properties.yaxisScaleFormatter(this, original);
                        else text = RGraph.numberFormat({
                            object: obj,
            ...
            var scale = obj.scale2;
            obj.maxLabelLength = Math.max(obj.maxLabelLength, obj.context.measureText(obj.scale2.labels[4]).width // * 2 // Don't know why this was doubled...?
            );
            //
            ...
            if (properties.xaxisPosition === "center") {
                ...
                for(var i1 = 0; i1 < scale.labels.length; ++i1){
                    ...
                    var y = properties.marginTop + section * i1;
                    RGraph.text({
                        object: obj,
                        ...
                        y: properties.yaxisScaleInvert ? halfHeight + properties.marginTop - section * i1 + properties.yaxisLabelsOffsety : properties.marginTop + section * i1 + properties.yaxisLabelsOffsety,
                        text: String(scale.labels[scale.labels.length - 1 - i1]),
                ...
                for(var i1 = 0; i1 < scale.labels.length; ++i1){
                    if (i1 === 0 && properties.yaxisScaleInvert) continue;
                    var section = (obj.canvas.height - properties.marginTop - properties.marginBottom) / (scale.labels.length * 2);
                    var y = properties.marginTop + (obj.canvas.height - properties.marginTop - properties.marginBottom) / 2 + section * i1 + section;
                    RGraph.text({
                        object: obj,
                        textConfPrefix: "yaxisLabels",
                        x: properties.yaxisPosition === "right" ? x + (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx + 5 : x - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx - 5,
                        y: properties.yaxisScaleInvert ? halfHeight + properties.marginTop + section * i1 : y + properties.yaxisLabelsOffsety,
                        text: "-" + (properties.yaxisScaleInvert ? String(scale.labels[scale.labels.length - i1 - 1]) : String(scale.labels[i1])),
                        valign: typeof properties.yaxisLabelsValign === "string" ? properties.yaxisLabelsValign : valign,
                        halign: typeof properties.yaxisLabelsHalign === "string" ? properties.yaxisLabelsHalign : properties.yaxisPosition === "right" ? "left" : halign,
                        marker: false,
                        angle: angle,
                ...
                RGraph.text({
                    ...
                    text: typeof properties.yaxisScaleFormatter === "function" ? properties.yaxisScaleFormatter({
                        object: this,
                        number: 0,
                        unitspre: properties.yaxisScaleUnitsPre,
                        unitspost: properties.yaxisScaleUnitsPost,
                    ...
                    }) : (properties.yaxisScaleUnitsPre || "") + properties.yaxisScaleMin.toFixed(properties.yaxisScaleDecimals).replace(/\./, properties.yaxisScalePoint) + (properties.yaxisScaleUnitsPost || ""),
                    valign: typeof properties.yaxisLabelsValign === "string" ? properties.yaxisLabelsValign : valign,
                    halign: typeof properties.yaxisLabelsHalign === "string" ? properties.yaxisLabelsHalign : properties.yaxisPosition === "right" ? "left" : halign,
                    marker: false,
                    angle: angle,
                ...
                //
                if (properties.yaxisScaleInvert) RGraph.text({
                    object: obj,
                    textConfPrefix: "yaxisLabels",
                    x: properties.yaxisPosition === "right" ? x + 5 + (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx : x - 5 - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx,
            ...
            //
                ...
                for(var i1 = 0; i1 < scale.labels.length; ++i1){
                    ...
                    if (properties.yaxisScaleInvert) var y = obj.canvas.height - properties.marginBottom - section * i1 - section - section;
                    else var y = properties.marginTop + section * i1;
                    RGraph.text({
                        object: obj,
                        textConfPrefix: "yaxisLabels",
                        x: properties.yaxisPosition === "right" ? x + (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx + 5 : x - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx - 5,
                        y: y + section + properties.yaxisLabelsOffsety,
                        text: "-" + String(scale.labels[i1]),
                        valign: typeof properties.yaxisLabelsValign === "string" ? properties.yaxisLabelsValign : valign,
                        halign: typeof properties.yaxisLabelsHalign === "string" ? properties.yaxisLabelsHalign : properties.yaxisPosition === "right" ? "left" : halign,
                        marker: false,
                        angle: angle,
            ...
            //
                ...
                for(var i1 = 0; i1 < scale.labels.length; ++i1){
                    var section = (obj.canvas.height - properties.marginTop - properties.marginBottom) / scale.labels.length;
                    var y = properties.marginTop + section * (i1 + (properties.yaxisScaleInvert ? 1 : 0));
                    RGraph.text({
                        object: obj,
                        textConfPrefix: "yaxisLabels",
                        x: properties.yaxisPosition === "right" ? x + (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx + 5 : x - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx - 5,
                        y: y + properties.yaxisLabelsOffsety,
                        text: String(properties.yaxisScaleInvert ? scale.labels[i1] : scale.labels[scale.labels.length - 1 - i1]),
                        valign: typeof properties.yaxisLabelsValign === "string" ? properties.yaxisLabelsValign : valign,
                        halign: typeof properties.yaxisLabelsHalign === "string" ? properties.yaxisLabelsHalign : properties.yaxisPosition === "right" ? "left" : halign,
                        marker: false,
                        angle: angle,
        ...
        //
            ...
            for(var i1 = 0; i1 < properties.yaxisLabelsSpecific.length; ++i1){
                var y = properties.marginTop + section * i1 + (properties.yaxisLabelsPosition === "section" ? section / 2 : 0);
                var ret = RGraph.text({
                    object: obj,
                    textConfPrefix: "yaxisLabels",
                    x: obj.type === "drawing.yaxis" ? properties.yaxisPosition === "right" ? obj.x + 7 + properties.yaxisLabelsOffsetx : obj.x - 5 + properties.yaxisLabelsOffsetx : properties.yaxisPosition === "right" ? x + properties.yaxisLabelsOffsetx + 5 : properties.marginLeft - 5 + properties.yaxisLabelsOffsetx,
                    y: y + properties.yaxisLabelsOffsety,
                    text: String(properties.yaxisLabelsSpecific[i1] || ""),
                ...
                obj.maxLabelLength = Math.max(obj.maxLabelLength, obj.context.measureText(String(properties.yaxisLabelsSpecific[i1])).width * 2);
            }
        }
        //
        // Draw the title
        ...
        if (properties.yaxisTitle) {
            ...
            // can be adjusted
            //
            if (obj.type === "gantt") for(var i1 = 0, maxLabelLength = 0; i1 < properties.yaxisLabels.length; ++i1){
                var textConf = RGraph.getTextConf({
                    object: obj,
                    prefix: "yaxisLabels"
                });
                maxLabelLength = Math.max(maxLabelLength, RGraph.measureText(properties.yaxisLabels[i1], textConf.bold, textConf.font, textConf.size)[0]);
            }
                ...
                var maxLabelLength = RGraph.measureText(obj.scale2.labels[obj.scale2.labels.length - 1], textConf.bold, textConf.font, textConf.size)[0];
            }
            ...
            if (obj.type === "hbar" && properties.yaxisLabels && properties.yaxisLabels.length || obj.type === "drawing.yaxis" && properties.yaxisLabelsSpecific && properties.yaxisLabelsSpecific.length) maxLabelLength = function(labels) {
                ...
                for(var i1 = 0, max = 0; i1 < labels.length; ++i1){
                    var dim = RGraph.measureText(labels[i1], textConf.bold, textConf.font, textConf.size);
            ...
            if (obj.type === "drawing.yaxis") var x = properties.yaxisPosition === "right" ? obj.x + 5 + maxLabelLength + 10 : obj.x - 5 - maxLabelLength - 10;
            // The yaxisTitlePos property
            if (typeof properties.yaxisTitlePos === "number") {
                if (properties.yaxisPosition === "right") x = obj.canvas.width - properties.marginRight * properties.yaxisTitlePos;
                else x = properties.marginLeft * properties.yaxisTitlePos;
            }
            // Specific X and Y coordinates for the title
    ...
    RGraph.getLabelsCSSClassName = function() {
        ...
        var properties = args.object.properties;
        var value = "";
        if (typeof properties[args.name] === "string") value = properties[args.name];
        else if (typeof properties[args.name] === "object" && typeof properties[args.name][args.index] === "string") value = properties[args.name][args.index];
        return value;
    };
    //
    ...
    RGraph.setCSS = function(element, properties) {
        ...
        for(i in properties)if (typeof i === "string") element.style[i] = properties[i];
    };
    //
    ...
    RGraph.labelSubstitution = function(args) {
        ...
        while(text.match(/%{value(?:_formatted)?}/i)){
            ...
            if (text.match(/%{value_formatted}/i)) text = text.replace("%{value_formatted}", typeof value === "number" ? RGraph.numberFormat({
                object: args.object,
    ...
    //
    RGraph.GET.text = RGraph.GET.string = function(key) {
    ...
    // @param string key The part to get
    //
    RGraph.GET.json = RGraph.GET.object = function(key) {
        var parts = RGraph.GET.parse();
        if (!parts[key]) return null;
    ...
    //                        comma)
    //
    RGraph.GET.list = RGraph.GET.array = function(key) {
        var parts = RGraph.GET.parse();
        if (!parts[key]) return null;
        if (!arguments[1]) var sep = ",";
        else var sep = arguments[1];
        ...
        for(var i1 = 0; i1 < arr.length; ++i1){
            ...
            arr[i1] = arr[i1].replace(/^('|")/, "");
            arr[i1] = arr[i1].replace(/('|")$/, "");
            if (Number(arr[i1])) arr[i1] = Number(arr[i1]);
        }
        return arr;
    };
    //
    ...
    RGraph.clipTo.start = function() {
        ...
        if (typeof args.dimensions === "string") {
            if (args.dimensions === "tophalf") args.object.path("sa b r % % % % cl", 0, 0, args.object.canvas.width, args.object.canvas.height / 2);
            else if (args.dimensions === "bottomhalf") args.object.path("sa b r % % % % cl", 0, args.object.canvas.height / 2, args.object.canvas.width, args.object.canvas.height / 2);
            else if (args.dimensions === "lefthalf") args.object.path("sa b r % % % % cl", 0, 0, args.object.canvas.width / 2, args.object.canvas.height);
            else if (args.dimensions === "righthalf") args.object.path("sa b r % % % % cl", args.object.canvas.width / 2, 0, args.object.canvas.width / 2, args.object.canvas.height);
            else if (args.dimensions.match(/^((?:h|v)bar) +([-.0-9]+) ?- ?([-.0-9]+)$/i)) {
                var type = RegExp.$1.toLowerCase(), start = parseFloat(RegExp.$2), end = parseFloat(RegExp.$3), marginTop = args.object.properties.marginTop, marginBottom = args.object.properties.marginBottom, marginLeft = args.object.properties.marginLeft, marginRight = args.object.properties.marginRight, graphWidth = args.object.canvas.width - marginLeft - marginRight, graphHeight = args.object.canvas.height - marginTop - marginBottom;
                if (type === "hbar") args.object.path("sa b r % % % % cl", 0, marginTop + graphHeight * start, args.object.canvas.width, (end - start) * (args.object.canvas.height - marginTop - marginBottom));
                else if (type === "vbar") args.object.path("sa b r % % % % cl", marginLeft + graphWidth * start, 0, (end - start) * (args.object.canvas.width - marginLeft - marginRight), args.object.canvas.height);
        ...
        // [[0,0],[0,250],[600,250],[600,0]]
            ...
            for(var i1 = 0, path = []; i1 < args.dimensions.length; ++i1)path.push("%1 %2 %3".format(i1 === 0 ? "m" : "l", args.dimensions[i1][0], args.dimensions[i1][1]));
        ...
        // (x/y/w/h)
        } else if (RGraph.isArray(args.dimensions)) args.object.path("sa b r % % % % cl", args.dimensions[0], args.dimensions[1], args.dimensions[2], args.dimensions[3]);
    };
    //
    ...
    RGraph.isBool = RGraph.isBoolean = function(obj) {
        return typeof obj === "boolean";
    };
    //RGraph.isArray Defined above
    RGraph.isObject = function(obj) {
    ...
    RGraph.isUndefined = function(obj) {
...
window.$d = function(m) {
    ...
    if (!this.rgraph_debug_textarea) {
        ...
        this.rgraph_debug_textarea.onmouseover = function(e) {
            e.target.style.opacity = 1;
        };
        this.rgraph_debug_textarea.onmousedown = function(e) {
            if (e.ctrlKey) {
                e.preventDefault();
                e.stopPropagation();
                this.mousedown = true;
                this.pickupX = e.offsetX;
                this.pickupY = e.offsetY;
                return false;
            }
        };
        this.rgraph_debug_textarea.onmouseout = function(e) {
            this.style.opacity = 0.25;
        };
        this.rgraph_debug_textarea.ondblclick = function(e) {
            if (confirm("Clear the log?")) this.value = "";
        };
        window.onmouseup = function(e) {
            this.rgraph_debug_textarea.mousedown = false;
        };
    ...
    function getCircularReplacer() {
        ...
        return function(key, value) {
            if (typeof value === "object" && value !== null) {
                ...
                for(var i1 = 0; i1 < seen.length; ++i1){
                    if (seen[i1] === value) return;
                }
                seen.push(value);
            }
            return value;
    ...
    this.rgraph_debug_textarea.value = "[%1:%2:%3] %4\r\n%5".format(hour, min, sec, JSON.stringify(m, getCircularReplacer()), this.rgraph_debug_textarea.value);
};
//
...
String.prototype.format = function() {
    ...
    if (arguments.length === 0) {
        ...
        if (s.match(/{[a-z0-9]+?}/i)) var s = this.replace(/{[a-z0-9]+?}/gi, function(str, idx) {
            str = str.substr(1);
    ...
    var args = arguments;
    for(var i1 in args)if (RGraph.isNull(args[i1])) args[i1] = "null";
...
//# sourceMappingURL=index.7eed6896.js.map
